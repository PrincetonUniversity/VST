...
Section generic_boolean.
  ...
  Lemma type_if_generic_boolean stn it (b : bool) v v'
     `{!TCEq (valinject it v) v'} T1 T2 :
     case_destruct b (λ b' _,
     li_trace (TraceIfBool b, b') (if b' then T1 else T2))
    ⊢ typed_if it v (v' ◁ᵥ|it| b @ generic_boolean stn it) ⎡valid_val v⎤ T1 T2.
  Proof.
    rewrite -TCEq0.
  ...
  Lemma type_assert_generic_boolean stn it v v' (b : bool) `{!TCEq (valinject it v) v'} R :
    (<affine> ⌜b⌝ ∗ T_normal R)
    ⊢ typed_assert it v (v' ◁ᵥ|it| b @ generic_boolean stn it) R.
  Proof.
    rewrite -TCEq0.
