...
Lemma age_sepcon {A}{JA: Join A}{PA: Perm_alg A}{agA: ageable A}{AgeA: Age_alg A}:
  forall P Q: pred A, 
   @msl.predicates_sa.sepcon A JA (app_pred P) (app_pred Q) = app_pred (P*Q).
Proof.
intros. extensionality w; apply prop_ext; split; intro H; hnf in *; auto. Qed.


Instance env_ag{key: Type}{A: Type} : ageable (env key A) := ag_flat (env key A).
Instance Age_env {key: Type}{A: Type}: @Age_alg (env key A) Join_env env_ag := asa_flat.

Section ENVSEC.
Context {key: Type}{A: Type}.

Lemma empty_env_unit:  forall rho: env key A, unit_for empty_env rho.
Proof. apply EnvSA.empty_env_unit. Qed.

Lemma empty_env_unit' : forall rho: env key A, join empty_env rho rho.
Proof. apply EnvSA.empty_env_unit'. Qed.
Hint Resolve @empty_env_unit @empty_env_unit'.

Lemma emp_empty_env :  forall rho: env key A, emp rho <-> rho = empty_env.
Proof. 
intros; destruct (EnvSL.emp_empty_env rho).
split; intros; auto.
subst. apply H0; auto.
Qed.

Lemma emp_empty_env' : emp (@empty_env key A).
Proof.
rewrite emp_empty_env.
auto.
Qed.
...
Program Definition env_mapsto{KE: EqDec key}  (id: key) (sh: Share.t) (v: A) :  pred (env key A) :=
        _env_mapsto id sh v.
Next Obligation. inv H. Qed.

Lemma env_mapsto_exists: forall {KE: EqDec key}  id sh v, exists rho, env_mapsto id (pshare_sh sh) v rho.
Proof. intros; destruct (_env_mapsto_exists id sh v) as [rho ?]. exists rho. apply H. Qed.

Lemma env_get_mapsto: forall {KE: EqDec key}  id v rho,  
  (exists sh, env_get rho id = Some (sh,v)) = 
  (exp (fun sh => env_mapsto id sh v) * TT)%pred rho.
Proof. intros. rewrite _env_get_mapsto. reflexivity. Qed.
 
Lemma env_get_mapsto': forall {KE: EqDec key}  id (sh: pshare) v rho, 
  env_get rho id = Some(pfullshare,v) -> 
       (env_mapsto id (pshare_sh sh) v * TT)%pred rho.
Proof. intros. generalize (_env_get_mapsto' id sh v rho H); intro. apply H0. Qed.

Lemma env_mapsto_set: forall {KE: EqDec key} id v, 
  env_mapsto id Share.top v (env_set id v empty_env).
Proof. intros. generalize (_env_mapsto_set id v); intro. apply H. Qed.

Lemma env_mapsto_get: forall {KE: EqDec key}  id sh v rho,
  env_mapsto id sh v rho
   -> exists Pf,
        env_get rho id = Some (exist nonunit sh Pf, v).
Proof. intros. destruct (@_env_mapsto_get _ _ _ id sh v rho) as [p ?]. apply H. exists p; auto. Qed.

Lemma env_mapsto_get_neq: forall {KE: EqDec key}  (id1 id2: key) (sh: Share.t) (v: A) rho,
  id1 <> id2 -> env_mapsto id1 sh v rho -> env_get rho id2 = None.
Proof. intros. eapply _env_mapsto_get_neq; eauto. apply H0. Qed.

Lemma env_mapsto_empty_env: forall {KE: EqDec key}  id v sh, ~(env_mapsto id sh v empty_env).
Proof. intros. intro. eapply _env_mapsto_empty_env; eauto. apply H. Qed.

Lemma env_mapsto_splittable: forall {KE: EqDec key}  id v (sh sh1 sh2: pshare) rho,
  join sh1 sh2 sh
  -> (env_mapsto id (pshare_sh sh) v rho
        <-> (env_mapsto id (pshare_sh sh1) v * env_mapsto id (pshare_sh sh2) v)%pred rho).
Proof. intros. apply (@_env_mapsto_splittable _ _ _ id v sh sh1 sh2 rho) in H. intuition. Qed.
 
Lemma env_mapsto_positive{KE: EqDec key}: forall id sh (v: A) rho, 
  env_mapsto id sh v rho -> nonidentity sh.
Proof.
  intros until rho.
  intro H; apply env_mapsto_get in H; destruct H.
...
Lemma env_mapsto_cohere{KE: EqDec key}: forall id sh1 (v1: A) sh2 v2, 
  (env_mapsto id sh1 v1 * TT) && (env_mapsto id sh2 v2 * TT) 
    |-- !!(v1=v2).
Proof. repeat intro. destruct H. 
   apply (@EnvSL.env_mapsto_cohere _ _ _ id sh1 v1 sh2 v2 a).
   rewrite <- @age_sepcon in *; split; auto.
Qed.

Lemma env_mapsto_precise{KE: EqDec key}: forall id sh (v:A), precise (env_mapsto id sh v).
Proof.
  intros; intro; intros.
  eapply EnvSL.env_mapsto_precise; eauto. apply H. apply H0.
Qed.

Definition own_var {KE: EqDec key} (sh: pshare) (id: key) : pred (env key A) :=
  exp (env_mapsto id (pshare_sh sh)).

Definition see_var {KE: EqDec key} (id: key) : pred (env key A) := 
  exp (fun sh: pshare => own_var sh id).

Definition list_sepcon {A} {JA: Join A}{PA: Perm_alg A}{SA: Sep_alg A}{agA: ageable A}{AgeA: Age_alg A}:
   list (pred A) -> pred A := fold_right sepcon emp.

Definition own_all {KE: EqDec key} (l: list key) : pred (env key A) := 
   list_sepcon (map (own_var pfullshare) l).

Lemma own_all_nil{KE: EqDec key} : own_all nil = (emp: pred (env key A)).
Proof. unfold own_all; simpl; auto. Qed.
