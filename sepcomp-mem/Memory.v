...
Require Export sepcomp.Memtype.
...
Hint Local Resolve perm_cur_max: mem.
...
Theorem range_perm_cur:
  forall m b lo hi p,
  range_perm m b lo hi Cur p -> range_perm m b lo hi Max p.
Proof.
  unfold range_perm; intros; eauto with mem.
Qed.

Hint Local Resolve range_perm_implies range_perm_cur: mem.
...
Lemma valid_access_max:
  forall m chunk b ofs p,
  valid_access m chunk b ofs p ->
  perm m b ofs Max p.
Proof.
  intros. destruct H. apply perm_cur_max. apply H. generalize (size_chunk_pos chunk). omega.
Qed.
  ...
  if range_perm_dec m b lo hi Cur Freeable then
    Some (mkmem m.(mem_contents)
                (ZMap.set b
                        (fun ofs k => 
                          if isMaxCur k then 
                            if zle lo ofs && zlt ofs hi then Some p 
                            else m.(mem_access)#b ofs k
                          else m.(mem_access)#b ofs k)
                        m.(mem_access))
                m.(nextblock) _ _ _)
...
Next Obligation.
  ...
  destruct k; try inv H2.
  destruct (zle lo ofs). destruct (zlt ofs hi).
  assert (perm m b ofs Max Freeable). apply perm_cur_max. apply H; auto. 
  unfold perm in H2. rewrite H1 in H2. contradiction.
  auto. auto. 
  destruct (zle lo ofs). destruct (zlt ofs hi).
  assert (perm m b ofs Cur Freeable). apply H; auto. 
  unfold perm in H2. rewrite H1 in H2. contradiction.
  auto. auto. 
...
Proof.
  ...
  destruct (zlt ofs lo).
  elim (perm_free_2 lo Cur p); auto.
  omega. apply H3. omega. 
  elim (perm_free_2 ofs Cur p); auto.
  omega. apply H3. omega. 
Qed.
...
Theorem perm_drop_1:
  forall ofs k, isMaxCur k=true -> lo <= ofs < hi -> perm m' b ofs k p.
Proof.
  intros.
  ...
  rewrite zle_true. rewrite zlt_true. simpl. 
  rewrite H.
  constructor. omega. omega. 
Qed.
  
Theorem perm_drop_2:
  forall ofs k p', isMaxCur k=true -> lo <= ofs < hi -> perm m' b ofs k p' -> perm_order p p'.
Proof.
  intros.
  unfold drop_perm in DROP. destruct (range_perm_dec m b lo hi Cur Freeable); inv DROP.
  revert H1. unfold perm; simpl. rewrite ZMap.gss. unfold proj_sumbool. 
  rewrite H. rewrite zle_true. rewrite zlt_true. simpl. auto. 
  omega. omega. 
Qed.
...
Proof.
  ...
  rewrite H; auto. rewrite H; auto. auto.
Qed.
...
Proof.
  intros.
  unfold drop_perm in DROP. destruct (range_perm_dec m b lo hi Cur Freeable); inv DROP.
  revert H0. unfold perm; simpl. rewrite ZMap.gsspec. destruct (ZIndexed.eq b' b).
  subst b'. unfold proj_sumbool. rewrite H. destruct (zle lo ofs). destruct (zlt ofs hi).
  simpl. intros. apply perm_implies with p. 
  destruct k; try inv H.
  apply perm_implies with Freeable. apply perm_cur_max. apply r. tauto. 
  auto with mem. 
  apply perm_implies with Freeable. apply r. tauto. auto with mem. auto.
  auto. auto. auto.
Qed.

Theorem perm_drop_5:
  forall b' ofs p', perm m' b' ofs Res p' -> perm m b' ofs Res p'.
Proof.
  intros.
  unfold drop_perm in DROP. destruct (range_perm_dec m b lo hi Cur Freeable); inv DROP.
  revert H. unfold perm; simpl. rewrite ZMap.gsspec. destruct (ZIndexed.eq b' b).
  subst b'; simpl; auto. auto.
Qed.
...
Theorem free_right_extends:
  ...
  extends m1 m2 ->
  free m2 b lo hi = Some m2' ->
  (forall ofs k p, isMaxCur k=true -> perm m1 b ofs k p -> lo <= ofs < hi -> False) ->
  extends m1 m2'.
Proof.
  intros. inv H. constructor.
  rewrite (nextblock_free _ _ _ _ _ H0). auto.
...
Proof.
  ...
  intros. inv H0. split; auto. 
  destruct a as [[b1 lo1] hi1].
  case_eq (free m b1 lo1 hi1); intros; try congruence.
  exploit IHl; eauto. intros [A B].
  split. eauto with mem.
  intros. destruct H3. inv H3.
  elim (perm_free_2 _ _ _ _ _ H ofs k p). auto. auto. auto.
  eauto.
Qed.
