...
Require Import sepcomp.core_semantics_lemmas. (*for definition of rely*)
...
Lemma reserved_dec: forall m b ofs, 
   {Mem.reserved m b ofs} +{~ Mem.reserved m b ofs}.
intros.
unfold Mem.reserved.
remember (Mem.perm_dec m b ofs Res Nonempty).
destruct s.
left. exists Nonempty. assumption.
right. intros N. destruct N.
  apply n. eapply Mem.perm_implies. apply H.
  constructor.
Qed.
...
Lemma II_ok: forall m1 m2 j12 (MInj12 : Mem.inject j12 m1 m2) m1'
                   ...
                   (Unch11': my_mem_unchanged_on  
                            (fun (b : block) (ofs : Z) => Mem.reserved m1 b ofs /\ ALLOC1 b ofs) 
                            m1 m1') 
                   (Unch33': my_mem_unchanged_on  
                            (fun (b : block) (ofs : Z) => Mem.reserved m3 b ofs /\ ALLOC3 b ofs) 
                            m3 m3') 
(*                   (Unch11': my_mem_unchanged_on 
                   ...
                   (AL13': inject_aligned j') ALLOC2
                   (ALL12: forall b1 b2 delta2, j12 b1 = Some(b2, delta2) ->
                            forall ofs, (ALLOC1 b1 (ofs-delta2) <-> ALLOC2 b2 ofs))
                   (ALL23: forall b2 b3 delta3, j23 b2 = Some(b3, delta3) ->
                            forall ofs, (ALLOC2 b2 (ofs-delta3) <-> ALLOC3 b3 ofs)), 
                j'=compose_meminj j12' j23' /\
                     ...
                     my_mem_unchanged_on  
                            (fun (b : block) (ofs : Z) => Mem.reserved m2 b ofs /\ ALLOC2 b ofs) 
                            m2 m2' /\
...
assert (UnchLOOR3: my_mem_unchanged_on (loc_out_of_reach j23 m2) m3 m3').
   ...
   eapply Unch33'. 
      ...
      apply H.
*)
assert (Minj13: Mem.inject (compose_meminj j12 j23) m1 m3).
   eapply Mem.inject_compose; eassumption.
assert (NOVj12':= RU_no_overlap _ _ _ MInj12 _ Fwd1 _ _ 
                  MInj23 _ _ _ _ _ HeqMKI).
assert (RELY2: my_mem_unchanged_on
                (fun b ofs => Mem.reserved m2 b ofs /\ ALLOC2 b ofs) m2 m2').
  split; intros b2 ofs2; intros.
      destruct HP as [Res2 Alloc2].
      apply (valid_split _ _ _ _ (ACCESS b2)); intros; clear ACCESS.
      (*case valid_block m2 b2*)
         clear H0.
         specialize (H1 k ofs2).
         case_eq (isMaxCur k); intros MCk.
         (*MaxCur*) 
         ...
         | Some (_, _) =>
             match source j12 m1 b2 ofs2 with
             | Some (b1, ofs1) =>
                 ZMap.get b2 (Mem.mem_access m2') ofs2 k =
                 ZMap.get b1 (Mem.mem_access m1') ofs1 k
             | None =>
                 ZMap.get b2 (Mem.mem_access m2') ofs2 k =
                 ZMap.get b2 (Mem.mem_access m2) ofs2 k
         ...
         | None =>
          ...
          clear H1 MCk.
             ...
             destruct d; apply eq_sym in Heqd.
               ...
               destruct ss.
               (*source  j12 m1 b2 ofs = Some p *)
                 destruct p.   
                 rewrite (perm_subst _ _ _ _ _ _ _ HH). clear HH.
                 destruct (source_SomeE _ _ _ _ _ Heqss)
                    as [b1 [delta2 [ofs1 [PP [VB [ JJ [PERM Off2]]]]]]].
                 clear Heqss. subst. inv PP. 
                 assert (VB1: Mem.valid_block m1 b1). apply VB.
                 assert (Res1: Mem.reserved m1 b1 ofs1).
                   destruct (Mem.mi_reserved _ _ _ MInj12 
                      b2 (ofs1+delta2) H) as [Res21 _].
                   specialize (Res21 Res2 _ _ VB1 JJ).
                   assert (ofs1 + delta2 - delta2 = ofs1) as -> by omega.
                   assumption.
                 assert (Alloc1: ALLOC1 b1 ofs1).
                   rewrite <- ALL12 in Alloc2; try eassumption.
                   assert (ofs1 + delta2 - delta2 = ofs1) as -> by omega.
                   assumption.
                 destruct Unch11' as [UPerm1 _].
                   rewrite <- UPerm1.
                      admit. (*UNCLEAR -- in mem_interpolation_II we could
                        derive a contradiction to PERM at this point...*)
                      split; assumption.
                      assumption.
               rewrite (perm_subst _ _ _ _ _ _ _ HH). trivial.
             rewrite (perm_subst _ _ _ _ _ _ _ HH). trivial.
        (*Res*)
          destruct k; inv MCk.
             rewrite (perm_subst _ _ _ _ _ _ _ H1). trivial.
     (*~ valid*) contradiction.
   (*Proof that values are unchaged is presumably*) admit.
           ...
           clear H2.
             ...
             rewrite (perm_subst _ _ _ _ _ _ _ HH). clear HH.
                 ...
                 destruct dd; apply eq_sym in Heqdd.
                   destruct p0.
                   rewrite (inc12 _ _ _ Heqdd) in H. inv H.
                   assert (VB1:= VBj12_1 _ _ _ Heqdd).
                   eapply MInj12; try eassumption.
                   rewrite UP; try assumption.
                     split. eapply (Mem.mi_unmappedreserved _ _ _ Minj13).
                            unfold compose_meminj. rewrite Heqdd.
                            rewrite Heqd. trivial.
                            apply (VBj12_1 _ _ _ Heqdd).
                     admit. (*UNCLEAR: WHERE SHOULD WE GET ALLOC1 b1 ofs FROM?
                       One possibility may be to add this axiom to
                         forward_simulation_inj: 
                       unmapped_alloc:
                          forall d j c1 m1 c2 m2, 
                            match_state d j c1 m1 c2 m2 ->  
                          forall b1, valid_block m1 b1 -> 
                                     j b1 = None ->
                          forall ofs, effects Sem1 c1 AllocEffect b1 ofs1*)

             destruct (sep12 _ _ _ Heqdd H) as [_ NV2]. contradiction.
        (*case ~ valid_block m2 b2*)
          specialize (H2 k (ofs+delta)).   
            assert (HH: 
...
(*                         assert (UNMAPPED: loc_unmapped 
                         ...
                         assert (UNCHANGED: Mem.reserved m1 b1 ofs /\ ALLOC1 b1 ofs).
                            split. eapply Minj13; try eassumption. 
                                    unfold compose_meminj. rewrite J12. rewrite J23. trivial.
                              admit. (*UNCLEAR: WHERE GET ALLOC1 FROM? 
                                  SAME ISSUE AS ABOVE*)
...
(*                         rewrite (Uval b1 ofs UNMAPPED RD _ (eq_refl _)).*)
                         rewrite (Uval b1 ofs UNCHANGED RD _ (eq_refl _)).
   ...
   (*unmappedreserved*)
     ...
     destruct o; apply eq_sym in Heqo.
       destruct p.
         ...
         subst; clear H1. rename b0 into b3.
         unfold removeUndefs in J1'.
         rewrite Heqd in J1'. rewrite Heqo in J1'. 
         rewrite H in J1'. inv J1'.
     clear H. eapply MInj13'. apply Heqo. apply H0.
   (*reserved*) intros b2 ofs VB2'.
      split; intros. unfold Mem.reserved in *.
         destruct H as [p Hp].
      apply (valid_split _ _ _ _ (ACCESS b2)); intros; clear ACCESS.
        (*case valid_block m2 b2*)
           specialize (H2 Res ofs). simpl in *.
           assert (J: j12 b1 = Some(b2, delta)).
             remember (j12 b1) as d.
               destruct d; apply eq_sym in Heqd.
                 destruct p0. rewrite (inc12 _ _ _ Heqd) in H1.
                 assumption.
               destruct (sep12 _ _ _ Heqd H1).
                 contradiction.
           rewrite (perm_subst _ _ _ _ _ _ _ H2) in Hp. clear H2. 
           assert (VB1:= VBj12_1 _ _ _ J).
           destruct (Mem.mi_reserved _ _ _ MInj12 b2 ofs H) as [X _].
        ...
        (*case ~ valid_block m2 b2*)
          ...
          destruct src.
            destruct (sourceP_SomeE _ _ _ _ _ _ _ Heqsrc)
                    as [bb1 [dd1 [ofs11 [PP [VB [ JJ [PERM Off2]]]]]]].
            clear Heqsrc. subst.
            unfold Mem.perm in Hp. rewrite H2 in Hp.
...
(*               unfold removeUndefs in H0.
      ...
      apply (valid_split _ _ _ _ (ACCESS b2)); intros; clear ACCESS.
        (*case valid_block m2 b2*)
           ...
           eapply MInj12; try eassumption.
           intros.
           destruct (Fwd1 _ H1) as [VB1' [_ ?]].           
           specialize (H _ _ VB1' (inc12 _ _ _ H2)).
           destruct H as [p Hp].
           apply H3 in Hp. exists p. apply Hp.
        (*case ~ valid_block m2 b2*)
          specialize (H1 Res ofs); simpl in *.   
          remember (sourceP
             (fun (b : block) (ofs : Z) => ~ Mem.perm m1' b ofs Res Nonempty)
          ...
          destruct src.
            destruct (sourceP_SomeE _ _ _ _ _ _ _ Heqsrc)
                    as [bb1 [dd1 [ofs11 [PP [VB [ JJ [PERM Off2]]]]]]].
            clear Heqsrc. subst.
            assert (VB1' : Mem.valid_block m1' bb1). apply VB.
             specialize (H _ _  VB1' JJ).
             destruct H as [p Hp].
             exfalso. apply PERM. 
          ...
          specialize (sourceP_NoneE _ _ _ _ _ _ Heqsrc). 
            ...
            destruct (zlt b2 (Mem.nextblock m2')).
              exists Nonempty. unfold Mem.perm. rewrite H1.
               constructor.
            exfalso. clear -g VB2'. unfold Mem.valid_block in VB2'. omega.
split; trivial.
split; trivial.
              ...
              (*assert (UNCH: loc_out_of_reach 
                  ...
                  (*case b2 <> b*)
                     ...
                     destruct NOV.
                           apply H2. trivial.
                           apply H2. omega.*)
              assert (UNCH: Mem.reserved m3 b3 (ofs + delta) /\ ALLOC3 b3 (ofs + delta)).
                split. eapply Minj13.
                          apply (VBj23_2 _ _ _ FF).
                          intros. admit. (*UNCLEAR HOW TO PROVE THIS???*)  
                admit. (*UNCLEAR HOW TO PROVE THIS???*)
          ...
          (*case Mem.valid_block m2 b2*)
             ...
             (*case source  j12 m1 b2 ofs2  = None *)
                ...
                assert (MX : Mem.perm m2 b2 ofs2 Max Nonempty).
                     eapply Mem.perm_cur_max. eapply Mem.perm_implies. 
                     apply Perm2. constructor.
                assert (UNCH3: Mem.reserved m3 b3 (ofs2+delta3) /\ 
                               ALLOC3 b3 (ofs2+delta3)).
                  admit. (*HOW TO GET THIS?*)
(*                assert (LOOR: loc_out_of_reach 
                ...
                rewrite (UVal _ _ UNCH3 Perm3 _ (eq_refl _)).
    ...
    (*mi_unmappedeserved*)
      ...
      apply (valid_split _ _ _ _ (ACCESS b2)); intros; clear ACCESS.
        ...
        (*case ~ valid_block m2 b2*)
          ...
          exfalso. clear -g VB2'. unfold Mem.valid_block in VB2'. omega.
    (*reserved*) intros b3 ofs VB3'.
      split. intros R3 b2 delta3 VB2' J2'. unfold Mem.reserved in *.
         destruct R3 as [p Hp].
      apply (valid_split _ _ _ _ (ACCESS b2)); intros; clear ACCESS.
        (*case valid_block m2 b2*)
           ...
           assert (J: j23 b2 = Some(b3, delta3)).
             remember (j23 b2) as d.
               ...
               destruct (sep23 _ _ _ Heqd J2').
                 contradiction.
           assert (VB3 := VBj23_2 _ _ _ J).
           apply (Fwd3 _ VB3) in Hp.
           destruct (Mem.mi_reserved _ _ _ MInj23 b3 ofs VB3)
              as [X _].
        ...
        (*case ~ valid_block m2 b2*)
          ...
          destruct src.
            destruct (sourceP_SomeE _ _ _ _ _ _ _ Heqsrc)
                    as [bb1 [dd1 [ofs11 [PP [VB [ JJ [PERM Off2]]]]]]].
            clear Heqsrc. subst.
            destruct (Mem.mi_reserved _ _ _ MInj13' b3 ofs VB3') as [X _].
          ...
          specialize (sourceP_NoneE _ _ _ _ _ _ Heqsrc). 
            ...
            destruct (zlt b2 (Mem.nextblock m2')).
              exists Nonempty. unfold Mem.perm.
              rewrite H0. constructor.
          exfalso. clear -g VB2'. unfold Mem.valid_block in VB2'.
            omega.
           ...
           assert (J: j12 b1 = Some(b2, delta2)).
             remember (j12 b1) as d.
               ...
               destruct (sep12 _ _ _ Heqd J1').
                 contradiction.
           destruct (Mem.mi_reserved _ _ _ MInj12 b2 (ofs - delta3) H0) as [X _].
        ...
        (*case ~ valid_block m2 b2*)
          ...
          destruct src.
            destruct (sourceP_SomeE _ _ _ _ _ _ _ Heqsrc)
                    as [bb1 [dd1 [ofs11 [PP [VB [ JJ [PERM Off2]]]]]]].
            clear Heqsrc. subst.
            unfold Mem.reserved in R2'. destruct R2' as [p Hp].
...
Definition AccessMap_II_Property (X: block -> Z -> bool)
           (j12 j23 j12' :meminj) (m1 m1' m2 : mem)
           (NB:Z) (AM:ZMap.t (Z -> perm_kind -> option permission)) :Prop :=
  forall b2, 
    (Mem.valid_block m2 b2 -> forall k ofs2,
      match k with Res => ZMap.get b2 AM ofs2 k  = 
                          ZMap.get b2 m2.(Mem.mem_access) ofs2 k
      | _ =>
       (if X b2 ofs2 then ZMap.get b2 AM ofs2 k  = 
                          ZMap.get b2 m2.(Mem.mem_access) ofs2 k 
       else
         match j23 b2 with 
         None => ZMap.get b2 AM ofs2 k = ZMap.get b2 m2.(Mem.mem_access) ofs2 k
...
Lemma II_ok: forall m1 m2 j12 (MInj12 : Mem.inject j12 m1 m2) m1'
                   ...
                   (Alloc2_dec: forall b ofs, {ALLOC2 b ofs} + {~ALLOC2 b ofs})
                   (Unch11': my_mem_unchanged_on  
                            (fun (b : block) (ofs : Z) => Mem.reserved m1 b ofs /\ ALLOC1 b ofs) 
                            m1 m1') 
                   (Unch33': my_mem_unchanged_on  
...
(*                   (Unch11': my_mem_unchanged_on 
                   ...
                   (CONT:  Content_II_Property j12 j12' j23' m1 m1' m2 
                                               (m2'.(Mem.mem_contents)))
                   (ACCESS: AccessMap_II_Property 
                                 (fun (b : block) (ofs : Z) => 
                                    if
                                    reserved_dec m2 b ofs then Alloc2_dec b ofs
                                    else false)
                                 j12 j23 j12' m1 m1' m2 
                                 n2' (m2'.(Mem.mem_access)))
                   (AL12: inject_aligned j12) (AL23: inject_aligned j23)
...
assert (Fwd2: mem_forward m2 m2').
  ...
  (*max*)
     destruct (ACCESS b2) as [Val2 _].
     specialize (Val2 H Max ofs). simpl in Val2.
     remember (reserved_dec m2 b2 ofs) as r.
     destruct r.
       remember (Alloc2_dec b2 ofs).
       destruct s; simpl in Val2.
          rewrite (perm_subst _ _ _ _ _ _ _ Val2) in H0; clear Val2.
          apply H0.
       remember (j23 b2) as jb.
       destruct jb; apply eq_sym in Heqjb.
...
assert (RELY2: my_mem_unchanged_on
  ...
  split; intros b2 ofs; intros.
      ...
      (*case valid_block m2 b2*)
         ...
         (*MaxCur*)
            assert (HH: if if reserved_dec m2 b2 ofs
            then proj_sumbool (Alloc2_dec b2 ofs)
            else false
         then
          ZMap.get b2 (Mem.mem_access m2') ofs k =
          ZMap.get b2 (Mem.mem_access m2) ofs k
          ...
          match j23 b2 with
          | Some (_, _) =>
              match source j12 m1 b2 ofs with
              | Some (b1, ofs1) =>
                  ZMap.get b2 (Mem.mem_access m2') ofs k =
                  ZMap.get b1 (Mem.mem_access m1') ofs1 k
              | None =>
          ...
          destruct k; auto. simpl in H2. inv H2.
          clear H1. 
          remember (reserved_dec m2 b2 ofs).
          destruct s; try contradiction.
          remember (Alloc2_dec b2 ofs).
          destruct s; try contradiction. 
          simpl in HH. clear Heqs Heqs0.
                 rewrite (perm_subst _ _ _ _ _ _ _ HH). clear HH. trivial.
        (*Res*)
          destruct k; inv H2.
             rewrite (perm_subst _ _ _ _ _ _ _ H1). trivial.
     (*~ valid*) contradiction.
   (*Values similar*) admit.
...
assert (Inj12': Mem.inject (removeUndefs j12 j' prej12') m1' m2').
    assert (Perm12': forall b1 b2 delta ofs k p,
        ...
        (*case valid_block m2 b2*)
           specialize (H2 k (ofs+delta)).
             assert (HH:if if reserved_dec m2 b2 (ofs + delta)
            then proj_sumbool (Alloc2_dec b2 (ofs + delta))
            else false
         then
          ZMap.get b2 (Mem.mem_access m2') (ofs + delta) k =
          ZMap.get b2 (Mem.mem_access m2) (ofs + delta) k
          ...
          | None =>
             ...
             assert (MAX: Mem.perm m1' b1 ofs Max Nonempty).
               ...
               eapply Mem.perm_implies; try eassumption.
                  constructor.
           remember (reserved_dec m2 b2 (ofs+delta)).
           destruct s; simpl in HH. clear Heqs.
             remember (Alloc2_dec b2 (ofs+delta)).
             destruct s; simpl in HH. clear Heqs.
               rewrite (perm_subst _ _ _ _ _ _ _ HH). clear HH. apply H0.
                 ...
                 destruct dd; apply eq_sym in Heqdd.
                   ...
                   rewrite UP; try assumption.
                     ...
                     split. eapply (Mem.mi_unmappedreserved _ _ _ Minj13).
                            ...
                            rewrite Heqd. trivial.
                            apply (VBj12_1 _ _ _ Heqdd).
                     admit. (*WHERE SHOULD WE GET ALLOC1 FROM?*)
             destruct (sep12 _ _ _ Heqdd H) as [_ NV2]. exfalso. apply (NV2 H1).
        (*case ~ valid_block m2 b2*)
          specialize (H2 k (ofs+delta)).   
            assert (HH: 
      ...
      (*valid*)
          ...
          (*source  j12 m1 b2 ofs = None*)
              ...
              assert (UNCH: Mem.reserved m3 b3 (ofs + delta) /\ ALLOC3 b3 (ofs + delta)).
                split. eapply Minj13.
                          apply (VBj23_2 _ _ _ FF).
                          intros. admit. (*HOW TO PROVE THIS???*)  
