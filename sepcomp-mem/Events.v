...
Proof.
  ...
  assert (EQ: Int.unsigned (Int.add ofs (Int.repr delta)) = Int.unsigned ofs + delta).
    ...
    eapply Mem.store_valid_access_1 in H5; eauto.
    destruct H5 as [H5 _]. eapply H5. split. omega. 
...
(* mem inject *)
  ...
  eauto. auto. 
  rewrite EQ. auto.  
  assert (Mem.free_list m1 ((b, Int.unsigned lo - 4, 
    Int.unsigned lo + Int.unsigned sz) :: nil) = Some m2).
  ...
  assert (k=Cur \/ k=Max) by (destruct k; try inv H8; auto).
  destruct H12; subst k.
  apply Mem.perm_cur_max; eauto.
