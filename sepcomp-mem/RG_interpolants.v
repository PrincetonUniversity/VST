...
(Sem2 : EffectfulSemantics (Genv.t F2 V2) C2 (list (ident * globdef F2 V2)))
                             ...
                             (WD3: mem_wd m3) (WD3' : mem_wd m3')
                              U1 U3
                             (Rely1: mem_unchanged_on U1 m1 m1')
                             (Rely3: mem_unchanged_on U3 m3 m3')
                             (HU1: forall b1 ofs b3 delta, U1 b1 ofs -> 
                                      compose_meminj j12 j23 b1 = Some(b3,delta) -> U3 b3 (ofs+delta))
(*                             (HU1: forall b1 ofs, U1 b1 ofs -> 
                                   exists b3 delta,  compose_meminj j12 j23 b1 = Some(b3,delta) /\ U3 b3 (ofs+delta))*)
(*                             (HU1: forall b ofs, U1 b ofs -> 
                                      reserved m1 b ofs /\ 
                                      exists b3 delta, compose_meminj j12 j23 b = Some(b3,delta) /\ R3 b3 (ofs+delta))*),
                 exists m2', exists j12', exists j23', j'=compose_meminj j12' j23' /\
                             ...
                             (mem_wd m2 -> mem_wd m2') /\
                         exists U2, mem_unchanged_on U2 m2 m2' /\
                             (forall b1 ofs b2 delta, U1 b1 ofs -> 
                                      j12 b1 = Some(b2,delta) -> U2 b2 (ofs+delta))
                          /\ (forall b2 ofs b3 delta, U2 b2 ofs -> 
                                      j23 b2 = Some(b3,delta) -> U3 b3 (ofs+delta)).
