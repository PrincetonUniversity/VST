...
Section Sim_EXT_SIMU_DIAGRAMS.
  ...
  Variable match_memwd:
    forall cd c1 m1 c2 m2, 
      match_states cd c1 m1 c2 m2 -> mem_wd m1 /\ mem_wd m2.
  ...
  Hypothesis ext_after_external:
      forall d st1 st2 m1 m2 e vals1 vals2 ret1 ret2 m1' m2' ef_sig,
        (d=st1 /\ match_states d st1 m1 st2 m2) ->
...
Proof.
  ...
  solve[exploit match_reserved; eauto].
  intros; destruct H0 as [? ?]; subst.
  exploit ext_simulation; eauto.  
  intros [c2' [m2' [? Step]]].
  exists c2'. exists m2'. exists st1'. 
  solve[split; auto].
  intros.
...
Section Sim_INJ_SIMU_DIAGRAMS.
  ...
  Variable match_memwd : 
    forall cd j c1 m1 c2 m2, 
      match_states cd j c1 m1 c2 m2 -> mem_wd m1 /\ mem_wd m2.

  Variable effects_valid : forall cd j c1 m1 c2 m2,
  ...
  Hypothesis inj_safely_halted:
    forall cd j c1 m1 c2 m2 v1 rty,
      match_states cd j c1 m1 c2 m2 ->
  ...
  Hypothesis inj_after_external:
      forall d j j' st1 st2 m1 e vals1 ret1 m1' m2 m2' ret2 sig,
        (d=st1 /\ match_states d j st1 m1 st2 m2) ->
        at_external Sem1 st1 = Some (e,sig,vals1) ->
        meminj_preserves_globals ge1 j -> 
...
Proof.
  ...
  intros [c2' [m2' [j' [? [? [? ?]]]]]].
  exists c2'. exists m2'. exists st1'. exists j'. 
  split; auto. 
...
Proof.
  ...
  intros [c2' [m2' [j' [? [? H8]]]]].
  destruct H8.
  exists c2', m2', j'; split; auto. split; auto. split; auto. 
  destruct H4; auto.
  destruct H4; auto.
Qed.
