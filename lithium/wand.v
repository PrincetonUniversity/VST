...
Section wand_val.
  ...
  Program Definition wand_val_ex ly P (ty : A → type) : type := {|
    ...
    ty_own β l :=
      ∃ v,  <affine> ⌜field_compatible ly [] l⌝ ∗
                        <affine> ⌜field_compatible ly [] v ⌝ ∗ l ↦_(tptr tvoid)[β] v ∗
  ...
  Qed. *)
  Next Obligation. Admitted. (* iIntros (??????->) "Hl". iDestruct "Hl" as (?) "[$ _]". Qed. *)
  Next Obligation. iIntros (??????) "H".
                   iDestruct "H" as (v) "(Hly1&Hly2&Hl&_)".
                   iMod (heap_mapsto_own_state_share with "Hl") as "Hl". eauto with iFrame. Qed.
  Next Obligation. iIntros (???????).
                   iIntros "Hl".
                   iDestruct "Hl" as (v) "(%&%&?&?)".
                   Check heap_mapsto_own_state_share.
                   
                   eauto with iFrame. Qed.
