...
Lemma gs_disjUR (n : nat) (x y : gset_disjUR string): ✓{n} y → x ≼ₒ{n} y → x ≼{n} y.
  Proof. intros Hv Hxy; destruct y; destruct Hxy; subst; try done. 
  Admitted.

Canonical Structure gset_disjUR_authR := @authR _ gs_disjUR.
...
Section type.
  Context `{!typeG Σ} `{!lockG Σ} {cs : compspecs}.
  ...
  Proof.
    ...
    iDestruct (own_valid_2 γ (● GSet s) (● GSet s0) with "[$H1] [$H2]") as "Hown".
  Admitted.
  ...
  Next Obligation. iIntros (A γ n x ty ot mt v ?) "Hl". Admitted. (* by iApply ty_size_eq. Qed. *)
  Next Obligation. iIntros (A γ n x ty ot mt l ? ?) "Hl". by iApply (ty_ref with "[$Hl]").
  Qed.
