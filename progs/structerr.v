...
Module Info.
  ...
  Definition source_file := "progs/structerr.c"%string.
...
Definition __res : ident := 57%positive.
Definition _f : ident := 58%positive.
...
Definition _p__1 : ident := 59%positive.
...
Definition f_f := {|
  ...
  fn_params := ((__res, (tptr (Tstruct _foo noattr))) :: nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body :=
Sskip
...
Definition f_g := {|
  ...
  fn_params := ((_p__1, (Tstruct _foo noattr)) :: nil);
  fn_vars := ((_p__1, (Tstruct _foo noattr)) :: nil);
  fn_temps := nil;
  fn_body :=
(Sassign (Evar _p__1 (Tstruct _foo noattr))
  (Etempvar _p__1 (Tstruct _foo noattr)))
|}.
...
(Ssequence
  (Ssequence
    (Scall None
      (Evar _g (Tfunction (Tcons (Tstruct _foo noattr) Tnil) tvoid
                 cc_default)) ((Evar _p (Tstruct _foo noattr)) :: nil))
    (Sreturn (Some (Econst_int (Int.repr 0) tint))))
  (Sreturn (Some (Econst_int (Int.repr 0) tint))))
|}.

Definition composites : list composite_definition :=
(Composite _foo Struct ((_x, tint) :: (_y, tint) :: nil) noattr :: nil).
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_p, Gvar v_p) :: (_f, Gfun(Internal f_f)) :: (_g, Gfun(Internal f_g)) ::
 (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _g :: _f :: _p :: ___builtin_debug ::
