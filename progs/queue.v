...
Definition _make_elem : ident := 65%positive.
...
Definition _fifo_empty : ident := 62%positive.
...
Definition _fifo_new : ident := 57%positive.
...
Definition _fifo_put : ident := 61%positive.
...
Definition _fifo_get : ident := 64%positive.
...
(Ssequence
  (Ssequence
    ...
    (Sset _p (Etempvar 70%positive (tptr tvoid))))
  (Ssequence
    (Sassign
      (Efield
        (Ederef (Etempvar _p (tptr (Tstruct _elem noattr)))
...
Definition f_main := {|
  ...
  fn_temps := ((_i, tint) :: (_j, tint) ::
               ...
               (74%positive, (tptr (Tstruct _elem noattr))) ::
               (73%positive, (tptr (Tstruct _elem noattr))) ::
               (72%positive, (tptr (Tstruct _elem noattr))) ::
               (71%positive, (tptr (Tstruct _fifo noattr))) :: nil);
  fn_body :=
(Ssequence
  (Ssequence
    (Scall (Some 71%positive)
      (Evar _fifo_new (Tfunction Tnil (tptr (Tstruct _fifo noattr))
                        cc_default)) nil)
    (Sset _Q (Etempvar 71%positive (tptr (Tstruct _fifo noattr)))))
  (Ssequence
    (Ssequence
