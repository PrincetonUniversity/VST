...
Fixpoint listrep (l: list val) (x: val) : mpred :=
 match l with
 | h::hs => 
    EX y:val, 
...
Lemma listrep_valid_pointer:
  forall l p,
   listrep l p |-- valid_pointer p.
Proof.
 destruct l; unfold listrep; fold listrep;
 intros; normalize.
...
Lemma listrep_isptr: forall l p, isptr p ->
  listrep l p = EX h: _, EX l': _, EX y: _, !! (l = h :: l') && data_at Tsh t_struct_list (h,y) p  *  listrep l' y.
Proof.
  intros.
  destruct l as [| h l']; apply pred_ext.
  + unfold listrep; simpl.
    entailer!.
  + Intros h l' y.
  + Exists h l'.
 ...
 ** in this case (EX l1  PROP(...)LOCAL(...)(SEP(...)).  *)
forward_while
   (EX l1: list val, EX l2 : list val, 
    EX w: val, EX v: val,
     PROP (l = rev l1 ++ l2)
...
rewrite (listrep_isptr l2) by auto.
Intros h l2' y.
forward. (* t = v->tail *)
forward. (* v->tail = w; *)
forward. (* w = v; *)
