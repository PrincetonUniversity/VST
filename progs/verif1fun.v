...
Definition sumlist_spec :=
 DECLARE P._sumlist
  ...
  PRE [ P._p OF (tptr P.t_struct_list)]  (fun rho => lseg LS sh (map Vint contents) (eval_id P._p rho) nullval)
  POST [ tint ]  local (fun rho => Vint (sum_int contents) = retval rho).

Definition reverse_spec :=
 DECLARE P._reverse
  WITH sh : share, contents : list int
  PRE  [ P._p OF (tptr P.t_struct_list) ] !! writable_share sh &&
              (fun rho => lseg LS sh (map Vint contents) (eval_id P._p rho) nullval)
  POST [ (tptr P.t_struct_list) ] (fun rho => lseg LS sh (rev (map Vint contents)) (retval rho) nullval).
...
Definition sumlist_Inv (sh: share) (contents: list int) : assert :=
          (EX cts: list int, 
            PROP () LOCAL (fun rho => Vint (Int.sub (sum_int contents) (sum_int cts)) = eval_id P._s rho)
            SEP ( TT ; (fun rho => lseg LS sh (map Vint cts) (eval_id P._t rho) nullval))).
...
forward_while (sumlist_Inv sh contents)
    (PROP() LOCAL (fun rho => sum_int contents = force_int (eval_id P._s rho)) SEP(TT)).
...
Definition reverse_Inv (sh: share) (contents: list int) : assert :=
          (EX cts1: list int, EX cts2 : list int,
            ...
            LOCAL ()
            SEP ((fun rho => lseg LS sh (map Vint cts1) (eval_id P._w rho) nullval);
                   (fun rho => lseg LS sh (map Vint cts2) (eval_id P._v rho) nullval))).
...
forward_while (reverse_Inv sh contents)
         (PROP() LOCAL () SEP(fun rho => lseg LS sh (map Vint (rev contents)) (eval_id P._w rho) nullval)).
