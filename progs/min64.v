...
Module Info.
  ...
  Definition source_file := "progs/min64.c".
...
Definition _j : ident := 58%positive.
Definition _main : ident := 60%positive.
Definition _min : ident := 57%positive.
Definition _minimum : ident := 59%positive.
Definition _n : ident := 55%positive.

Definition f_minimum := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_a, (tptr tint)) :: (_n, tlong) :: nil);
  fn_vars := nil;
  fn_temps := ((_i, tlong) :: (_min, tint) :: (_j, tint) :: nil);
  fn_body :=
(Ssequence
  (Sset _min
    (Ederef
  ...
  (Ssequence
    (Ssequence
      ...
      (Sloop
        (Ssequence
          (Sifthenelse (Ebinop Olt (Etempvar _i tlong) (Etempvar _n tlong)
                         tint)
            Sskip
            Sbreak)
          (Ssequence
            (Sset _j
              (Ederef
                (Ebinop Oadd (Etempvar _a (tptr tint)) (Etempvar _i tlong)
                  (tptr tint)) tint))
            (Ssequence
              (Sassign
                (Ederef
