...
Definition _struct_foo : ident := 36%positive.
...
Definition _x1 : ident := 39%positive.
Definition ___builtin_membar : ident := 11%positive.
Definition _s : ident := 37%positive.
...
Definition _z : ident := 33%positive.
...
Definition v_s := {|
  ...
  gvar_init := (Init_int32 (Int.repr 5) ::
                Init_float32 (Float32.of_bits (Int.repr 14302577)) ::
...
Definition f_main := {|
  ...
  fn_body :=
(Ssequence
  (Sset _y1 (Ecast (Efield (Evar _s t_struct_foo) _y tfloat) tdouble))
  (Ssequence
