...
Definition _x1 : ident := 15%positive.
...
Definition _main : ident := 16%positive.
Definition _s : ident := 13%positive.
Definition ___builtin_mull : ident := 6%positive.
Definition _struct_foo : ident := 12%positive.
Definition _x : ident := 11%positive.
Definition ___builtin_subl : ident := 5%positive.
Definition ___builtin_write32_reversed : ident := 2%positive.
Definition _y : ident := 10%positive.

Definition t_struct_foo :=
   (Tstruct _struct_foo (Fcons _x tint (Fcons _y tfloat Fnil)) noattr).
...
Definition f_main := {|
  ...
  fn_params := nil;
  fn_vars := nil;
  fn_temps := ((_y1, tfloat) :: (_x1, tint) :: nil);
  fn_body :=
(Ssequence
  (Sset _y1 (Ecast (Efield (Evar _s t_struct_foo) _y tfloat) tfloat))
  (Ssequence
