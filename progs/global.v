...
Module Info.
  ...
  Definition source_file := "progs/global.c"%string.
...
Definition _d : ident := ltac:(string2ident "d"%string).
Definition _e : ident := ltac:(string2ident "e"%string).
...
Definition _x : ident := 3%positive.
Definition _y : ident := 4%positive.
Definition _t'1 : ident := 5%positive.
...
Definition v_c := {|
  gvar_info := (Tstruct _foo noattr);
  gvar_init := (Init_int32 (Int.repr 5) :: Init_space 2 :: Init_space 2 ::
                nil);
  gvar_readonly := false;
  gvar_volatile := false
|}.
...
(Composite _foo Struct ((_x, tint) :: (_a, (tarray tschar 2)) :: nil) noattr ::
 nil).
