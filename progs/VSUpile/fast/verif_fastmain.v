...
Lemma decreasing_inc i (I:0 <= i):
  i + 1 :: spec_fasttriang.decreasing (Z.to_nat i) = spec_fasttriang.decreasing (Z.to_nat (i + 1)).
  Proof. 
    replace (Z.to_nat (i+1)) with (S (Z.to_nat i)).
    unfold spec_fasttriang.decreasing; fold spec_fasttriang.decreasing.
...
sep_apply (spec_fastapile.make_apile APILE gv). (* yields spec_apile.apile APILE gv []; *)
(*sep_apply (make_apile PILE gv). yields apile PILE gv []; *)
generalize (spec_fastonepile.make_onepile ONEPILE gv).
(*generalize (make_onepile PILE gv).*)

assert (change_composite_env (spec_fastonepile.OnePileCompSpecs ONEPILE) LinkedCompSpecs).
make_cs_preserve (spec_fastonepile.OnePileCompSpecs ONEPILE) LinkedCompSpecs.
...
forward_for_simple_bound 10
  (EX i:Z,
   PROP() LOCAL(gvars gv)
   SEP (spec_fastonepile.onepile ONEPILE gv (Some (spec_fasttriang.decreasing (Z.to_nat i)));
          spec_fastapile.apile APILE gv (spec_fasttriang.decreasing (Z.to_nat i));
          spec_stdlib.mem_mgr M gv; has_ext tt)).
- 
 entailer!.  simpl; cancel.
-
unfold APILE.
forward_call (i+1, spec_fasttriang.decreasing(Z.to_nat i), gv).
{ simpl; cancel. }
rep_omega.
forward_call (i+1, spec_fasttriang.decreasing(Z.to_nat i), gv).
...
-
unfold APILE.
forward_call (spec_fasttriang.decreasing (Z.to_nat 10), gv).
{ simpl; cancel. }
