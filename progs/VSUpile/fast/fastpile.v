    ...
    (Sassign
      (Efield
        (Ederef (Etempvar _p (tptr (Tstruct _pile noattr)))
          (Tstruct _pile noattr)) _sum tint) (Econst_int (Int.repr 0) tint))
...
Definition f_Pile_add := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_pp, (tptr (Tstruct _pile noattr))) :: (_n, tint) :: nil);
  fn_vars := nil;
...
(Ssequence
  (Sset _s
    (Efield
      (Ederef (Etempvar _pp (tptr (Tstruct _pile noattr)))
        (Tstruct _pile noattr)) _sum tint))
  (Ssequence
...
Definition f_Pile_count := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_pp, (tptr (Tstruct _pile noattr))) :: nil);
  fn_vars := nil;
...
(Composite _pile Struct ((_sum, tint) :: nil) noattr :: nil).
