...
Definition _Apile_add : ident := 73%positive.
Definition _Apile_count : ident := 74%positive.
Definition _Onepile_add : ident := 77%positive.
Definition _Onepile_count : ident := 78%positive.
Definition _Onepile_init : ident := 76%positive.
Definition _Pile_add : ident := 66%positive.
Definition _Pile_count : ident := 69%positive.
Definition _Pile_free : ident := 71%positive.
Definition _Pile_new : ident := 65%positive.
...
Definition _a_pile : ident := 72%positive.
Definition _auxdata : ident := 87%positive.
Definition _c : ident := 68%positive.
Definition _c1 : ident := 81%positive.
Definition _c2 : ident := 82%positive.
Definition _c3 : ident := 83%positive.
Definition _exit : ident := 55%positive.
Definition _free : ident := 54%positive.
Definition _global_auxdata : ident := 90%positive.
Definition _head : ident := 61%positive.
Definition _i : ident := 79%positive.
Definition _incr1 : ident := 88%positive.
Definition _incr2 : ident := 89%positive.
Definition _incr3 : ident := 91%positive.
Definition _incr4 : ident := 92%positive.
Definition _list : ident := 59%positive.
Definition _main : ident := 57%positive.
Definition _malloc : ident := 53%positive.
Definition _n : ident := 58%positive.
Definition _next : ident := 60%positive.
Definition _p : ident := 63%positive.
Definition _pile : ident := 62%positive.
Definition _placeholder : ident := 56%positive.
Definition _pp : ident := 85%positive.
Definition _q : ident := 67%positive.
Definition _r : ident := 70%positive.
Definition _s : ident := 86%positive.
Definition _sum : ident := 84%positive.
Definition _surely_malloc : ident := 64%positive.
Definition _the_pile : ident := 75%positive.
Definition _t'1 : ident := 93%positive.

Definition f_incr1 := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_i, tint) :: (_auxdata, (tptr tuint)) :: nil);
  fn_vars := nil;
...
(Ssequence
  (Ssequence
    (Sset _t'1
      (Ederef
        (Ebinop Oadd (Etempvar _auxdata (tptr tuint))
          (Ebinop Omod (Etempvar _i tint) (Econst_int (Int.repr 10) tint)
...
(Ssequence
  (Ssequence
    (Sset _t'1
      (Ederef
        (Ebinop Oadd (Evar _global_auxdata (tarray tuint 10))
          (Ebinop Omod (Etempvar _i tint) (Econst_int (Int.repr 10) tint)
            tint) (tptr tuint)) tuint))
    (Sassign
      (Ederef
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_incr1, Gfun(Internal f_incr1)) :: (_incr2, Gfun(Internal f_incr2)) ::
 (_global_auxdata, Gvar v_global_auxdata) ::
 (_incr3, Gfun(Internal f_incr3)) :: (_incr4, Gfun(Internal f_incr4)) :: nil).

Definition public_idents : list ident :=
(_incr4 :: _incr3 :: _global_auxdata :: _incr2 :: _incr1 ::
