...
Program Definition incr_spec :=
 DECLARE _incr
  ...
  PRE [ ]
         ...
         LOCAL (gvars gv)
         SEPS   (lock_inv sh (gv _ctr_lock) (sync_inv g (ctr_state (gv _ctr)))) | (public_half g n)
  POST [ tvoid ]
         PROP ()
         LOCAL ()
...
Proof.
  ...
  forward.
  rewrite add_repr.
  forward_call [nat : Type] release_sync (gv _ctr_lock, sh, g, ctr_state (gv _ctr), n, (n + 1)%nat, Q, inv_names).
  { unfold ctr_state.
    subst Frame; instantiate (1 := nil); simpl.
    rewrite Nat2Z.inj_add; cancel.
    apply atomic_shift_derives_simple; try solve [iIntros; auto].
    iIntros (x _) "[? [[% ?] ?]]"; subst; iFrame; auto. }
  forward.
Qed.
...
Proof.
  ...
  gather_SEP 0 1 3 5; viewshift_SEP 0 (EX i, |> (wsat * invariant i (cptr_lock_inv g g1 g2 ctr lock))).
  { go_lower.
    apply make_inv'.
    unfold cptr_lock_inv.
    Exists O O; unfold ctr_inv; simpl; cancel. }
  Intros i.
  forward_call [nat : Type; nat : Type] release_inv (lock, g, O, ctr_state ctr, fun n n' => !!(n = 2 /\ n' = 2)%nat && lock_inv' lock g n (ctr_state ctr),
  ...
  { unfold ctr_state; cancel.
    ...
    unfold atomic_shift.
    iExists emp; iSplit; auto.
    iApply (andp_derives with "I"); auto.
    unfold ashift.
    iIntros "I >_".
    iMod (inv_open with "I") as "[>c H]"; auto.
    unfold cptr_lock_inv at 1.
    iDestruct "c" as (x y) "[g c]"; iExists (x + y)%nat; iFrame "c".
    iMod (updates.fupd_intro_mask (⊤ ∖ inv i) ∅ emp with "[]") as "mask"; auto.
    { apply empty_subseteq. }
    iIntros "!>"; iSplit.
  ...
  sep_apply cored_dup.
  rewrite <- add_andp by apply invariant_cored.
  ...
  forward_call (g, gv, fun n => !!(n = 2)%nat, inv_names).
  { rewrite -> 4sepcon_assoc; apply sepcon_derives; cancel.
