...
Module Info.
  ...
  Definition source_file := "fastpile_object.c"%string.
  Definition normalized := true.
End Info.

Definition _Apile_add : ident := 75%positive.
Definition _Apile_count : ident := 76%positive.
Definition _Fastpile_add : ident := 103%positive.
Definition _Fastpile_count : ident := 104%positive.
Definition _Fastpile_free : ident := 105%positive.
Definition _Fastpile_new : ident := 102%positive.
Definition _Onepile_add : ident := 72%positive.
Definition _Onepile_count : ident := 73%positive.
Definition _Onepile_init : ident := 71%positive.
Definition _PM_add : ident := 87%positive.
Definition _PM_count : ident := 88%positive.
Definition _PM_free : ident := 89%positive.
Definition _PM_new : ident := 86%positive.
Definition _P_add : ident := 97%positive.
Definition _P_count : ident := 98%positive.
Definition _P_free : ident := 99%positive.
Definition _P_new : ident := 95%positive.
...
Definition _Triang_nth : ident := 78%positive.
...
Definition _a_pile : ident := 74%positive.
Definition _c : ident := 65%positive.
Definition _c1 : ident := 79%positive.
Definition _c2 : ident := 80%positive.
Definition _c3 : ident := 81%positive.
Definition _data : ident := 92%positive.
Definition _exit : ident := 59%positive.
Definition _fastpile : ident := 101%positive.
...
Definition _meth : ident := 91%positive.
...
Definition _pile_data : ident := 85%positive.
Definition _pile_methods : ident := 90%positive.
Definition _pile_object : ident := 93%positive.
Definition _pile_pm : ident := 100%positive.
Definition _placeholder : ident := 84%positive.
Definition _pm : ident := 94%positive.
Definition _po : ident := 96%positive.
...
Definition f_Fastpile_new := {|
  fn_return := (tptr (Tstruct _fastpile noattr));
  fn_callconv := cc_default;
  fn_params := nil;
  fn_vars := nil;
  fn_temps := ((_p, (tptr (Tstruct _fastpile noattr))) ::
               (_t'1, (tptr tvoid)) :: nil);
  fn_body :=
(Ssequence
  (Ssequence
    (Scall (Some _t'1)
      (Evar _surely_malloc (Tfunction (Tcons tuint Tnil) (tptr tvoid)
                             cc_default))
      ((Esizeof (Tstruct _fastpile noattr) tuint) :: nil))
    (Sset _p
      (Ecast (Etempvar _t'1 (tptr tvoid)) (tptr (Tstruct _fastpile noattr)))))
  (Ssequence
    (Sassign
      (Efield
        (Ederef (Etempvar _p (tptr (Tstruct _fastpile noattr)))
          (Tstruct _fastpile noattr)) _sum tint)
      (Econst_int (Int.repr 0) tint))
    (Sreturn (Some (Etempvar _p (tptr (Tstruct _fastpile noattr)))))))
|}.

Definition f_Fastpile_add := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_p, (tptr (Tstruct _fastpile noattr))) :: (_n, tint) :: nil);
  fn_vars := nil;
...
(Ssequence
  ...
  (Ssequence
    (Sifthenelse (Ebinop Ole (Econst_int (Int.repr 0) tint)
      ...
      (Sset _t'1
        (Ecast
          (Ebinop Ole (Etempvar _n tint)
            (Ebinop Osub (Econst_int (Int.repr 2147483647) tint)
              (Etempvar _s tint) tint) tint) tbool))
    ...
    (Sifthenelse (Etempvar _t'1 tint)
      (Sassign
        (Efield
          (Ederef (Etempvar _p (tptr (Tstruct _fastpile noattr)))
            (Tstruct _fastpile noattr)) _sum tint)
        (Ebinop Oadd (Etempvar _s tint) (Etempvar _n tint) tint))
      Sskip)))
|}.

Definition f_Fastpile_count := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_p, (tptr (Tstruct _fastpile noattr))) :: nil);
  fn_vars := nil;
...
(Scall None
  ...
  ((Etempvar _p (tptr (Tstruct _fastpile noattr))) :: nil))
|}.

Definition v_pile_pm := {|
  gvar_info := (Tstruct _pile_methods noattr);
  gvar_init := (Init_addrof _Fastpile_new (Ptrofs.repr 0) ::
                Init_addrof _Fastpile_add (Ptrofs.repr 0) ::
                Init_addrof _Fastpile_count (Ptrofs.repr 0) ::
                Init_addrof _Fastpile_free (Ptrofs.repr 0) :: nil);
  gvar_readonly := false;
  gvar_volatile := false
|}.

Definition composites : list composite_definition :=
(Composite _pile_methods Struct
   ((_PM_new,
     (tptr (Tfunction Tnil (tptr (Tstruct _pile_data noattr)) cc_default))) ::
    (_PM_add,
     (tptr (Tfunction
             (Tcons (tptr (Tstruct _pile_data noattr)) (Tcons tint Tnil))
             tvoid cc_default))) ::
    (_PM_count,
     (tptr (Tfunction (Tcons (tptr (Tstruct _pile_data noattr)) Tnil) tint
             cc_default))) ::
    (_PM_free,
     (tptr (Tfunction (Tcons (tptr (Tstruct _pile_data noattr)) Tnil) tvoid
             cc_default))) :: nil)
   noattr :: Composite _fastpile Struct ((_sum, tint) :: nil) noattr :: nil).
...
((___builtin_bswap,
 ...
 (_Fastpile_new, Gfun(Internal f_Fastpile_new)) ::
 (_Fastpile_add, Gfun(Internal f_Fastpile_add)) ::
 (_Fastpile_count, Gfun(Internal f_Fastpile_count)) ::
 (_Fastpile_free, Gfun(Internal f_Fastpile_free)) ::
 (_pile_pm, Gvar v_pile_pm) :: nil).

Definition public_idents : list ident :=
(_pile_pm :: _Fastpile_free :: _Fastpile_count :: _Fastpile_add ::
 _Fastpile_new :: _exit :: _free :: _malloc :: ___builtin_debug ::
