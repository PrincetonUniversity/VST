...
Definition _b : ident := 47%positive.
...
Definition _do_and : ident := 49%positive.
...
Definition _do_or : ident := 48%positive.
...
Definition f_do_or := {|
  ...
  fn_vars := nil;
  fn_temps := ((51%positive, tint) :: nil);
  fn_body :=
(Ssequence
  (Sifthenelse (Etempvar _a tbool)
    (Sset 51%positive (Econst_int (Int.repr 1) tint))
    (Sset 51%positive (Ecast (Etempvar _b tbool) tbool)))
...
Definition f_do_and := {|
  ...
  fn_params := ((_a, tbool) :: (_b, tbool) :: nil);
  fn_vars := nil;
  fn_temps := ((52%positive, tint) :: nil);
  fn_body :=
(Ssequence
  (Sifthenelse (Etempvar _a tbool)
    (Sset 52%positive (Ecast (Etempvar _b tbool) tbool))
    (Sset 52%positive (Econst_int (Int.repr 0) tint)))
...
prog_public :=
(_main :: _do_and :: _do_or :: ___builtin_write32_reversed ::
