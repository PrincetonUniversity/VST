...
Lemma zero_in_body: forall i ls, ~In 0 ls -> Znth i (ls ++ [0]) 0 = 0 -> 0 <= i < Zlength ls -> forall P, P.
Proof.
intros.
contradiction H.
rewrite app_Znth1 in H0 by omega. rewrite <- H0. apply Znth_In. auto.
Qed.
...
forward_if.
forward.
entailer!. f_equal. f_equal. zero_in_body.
forward.
Exists (i+1).
entailer!. zero_in_body.
*
Intros i.
forward.
Exists i.
...
- Intros i. 
  ...
  { forward. 
    Exists (offset_val i str).
    entailer!.
    left. exists i. split3; auto. rewrite app_Znth1; auto. zero_in_body. }
  { forward.
    entailer!. }
  Intros.
  forward_if.
  { forward.
    ...
    right. split; auto.
    assert (i = Zlength ls) by zero_in_body.
    subst i.
    autorewrite with sublist in H3; auto. }
  forward.
  Exists (i+1); entailer!.
...
-
  ...
  +
    forward.
    Exists (i+1); entailer!. zero_in_body.
- Intros i.
   forward.
   Exists i. entailer!. 
-
  ...
  + forward.
      ...
      cancel.
    assert (j = Zlength ls) by zero_in_body; subst.
    autorewrite with sublist.
    apply derives_refl'.
 ...
 +
  forward.
  Exists (j+1).
  destruct (zlt j (Zlength ls)); [ | zero_in_body].
  entailer!.
...
- Intros i.
  ...
  { split; [|intro; subst; rewrite app_Znth2, Zminus_diag by omega; auto].
    ...
    intro X; lapply (Znth_In i ls1 0); [|omega]. zero_in_body. }
  assert (Znth i (ls2 ++ [0]) 0 = 0 <-> i = Zlength ls2) as Hs2.
  { split; [|intro; subst; rewrite app_Znth2, Zminus_diag by omega; auto].
    destruct (zlt i (Zlength ls2)); [|omega].
    intro X; lapply (Znth_In i ls2 0); [|omega]. zero_in_body. }
  forward.
  forward.
  ...
  { forward.
    ...
    + rewrite e; simpl force_val.
         assert (i = Zlength ls2) by zero_in_body.
...
+ forward.
   entailer!.
  assert (i = Zlength ls) by zero_in_body. subst i.
  change (field_at Tsh (tarray tschar n) []) with (data_at Tsh (tarray tschar n)).
  rewrite upd_Znth_app2 by list_solve.
  autorewrite with sublist.
  rewrite !map_app.
