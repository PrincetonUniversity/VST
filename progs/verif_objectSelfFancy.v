...
(*∃ACT SAME PROOF SCRIPT AS Lemma body_make_fancyfoo*)
...
+ clear - H2. unfold field_at; simpl; entailer!!.
  - unfold field_compatible. destruct H2 as [_ [_ [SZ [AL _]]]].
    ...
    ++ simpl. left; auto.
  - unfold at_offset. entailer!!.
+ clear. unfold field_at; simpl; entailer!!.
...
Definition main_spec :=
 DECLARE _main
  ...
  POST [ tint ]
     ∃ i:Z, PROP(0<=i<=6) LOCAL (temp ret_temp (Vint (Int.repr (i+13)))) SEP(True).

Notation funspec := (@funspec Σ).

Definition reset_intersection: funspec.
Proof.
eapply (binary_intersection'(A1 := ConstType _)(A2 := ConstType _) (reset_spec foo_obj_invariant) (freset_spec fancyfoo_obj_invariant)); reflexivity.
Defined.
...
Proof. eapply funspec_sub_trans. apply reset_spec_local_sub. unfold fobject_invariant_of_inv.
 do_funspec_sub.
  rewrite fancyfoo_obj_invariant_fold_unfold'. (*foo_obj_invariant_fold_unfold;*) Intros m.
  rewrite -fupd_intro.
...
Lemma funspec_sub_reset_foo_fancy: funspec_sub (reset_spec foo_obj_invariant) (freset_spec fancyfoo_obj_invariant).
Proof. do_funspec_sub. simpl in H. inv H.
  ...
  unfold fobject_methods.
  rewrite bi.later_exist; Intros sh.
  rewrite bi.later_exist; Intros r.
  rewrite bi.later_exist; Intros t.
  rewrite bi.later_exist; Intros tR.
  rewrite bi.later_exist; Intros sC.
  rewrite bi.later_exist; Intros gC.
  Exists ((
  ...
  rewrite bi.later_and !bi.later_sep. Intros.
  entailer. apply bi.and_intro.
