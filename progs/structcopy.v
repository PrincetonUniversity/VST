...
Module Info.
  ...
  Definition source_file := "progs/structcopy.c"%string.
...
Definition _f : ident := ltac:(string2ident "f"%string).
Definition _foo : ident := ltac:(string2ident "foo"%string).
Definition _main : ident := 5%positive.
Definition _p : ident := 6%positive.
Definition _q : ident := 7%positive.
Definition _t'1 : ident := 8%positive.
Definition _t'2 : ident := 9%positive.

Definition f_f := {|
  fn_return := tuint;
  fn_callconv := cc_default;
  fn_params := ((_p, (tptr (Tstruct _foo noattr))) :: nil);
  fn_vars := ((_q, (Tstruct _foo noattr)) :: nil);
  fn_temps := ((_t'2, tuint) :: (_t'1, tuint) :: nil);
  fn_body :=
(Ssequence
  (Sassign (Evar _q (Tstruct _foo noattr))
    (Ederef (Etempvar _p (tptr (Tstruct _foo noattr))) (Tstruct _foo noattr)))
  (Ssequence
    (Sset _t'1 (Efield (Evar _q (Tstruct _foo noattr)) _a tuint))
    (Ssequence
      (Sset _t'2 (Efield (Evar _q (Tstruct _foo noattr)) _b tuint))
      (Sreturn (Some (Ebinop Oadd (Etempvar _t'1 tuint) (Etempvar _t'2 tuint)
                       tuint))))))
|}.

Definition composites : list composite_definition :=
(Composite _foo Struct ((_a, tuint) :: (_b, tuint) :: nil) noattr :: nil).
...
(_f :: ___builtin_debug :: ___builtin_nop :: ___builtin_write32_reversed ::
