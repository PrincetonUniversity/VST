...
Lemma as_inj_alloc_left_sm_same: forall mu b1 b2 delta (WD: SM_wd mu)
...
Lemma as_inj_alloc_left_sm_other: forall mu b1 b2 delta b (H: b<>b1),
...
Proof. intros.
  ...
  rewrite as_inj_alloc_left_sm_other; trivial.
Qed.
...
Proof.
  ...
  assert (Mem.mem_inj (as_inj mu') m1 m2).
    ...
    rewrite as_inj_alloc_left_sm_other in H8; trivial.
      eauto.
...
(* inj *)
  ...
  unfold mu'; simpl. rewrite as_inj_alloc_left_sm_same; trivial.
(* freeblocks *)
...
(* mappedblocks *)
  ...
  destruct (eq_block b b1). 
    subst. rewrite as_inj_alloc_left_sm_same in H9; trivial.
     congruence. eauto.
  rewrite as_inj_alloc_left_sm_other in H9; trivial.
    eauto.
(* overlap *)
  ...
  destruct (eq_block b0 b1); try subst b0; destruct (eq_block b3 b1); try subst b3. 
    elim H9; trivial.
    rewrite as_inj_alloc_left_sm_same in H10; trivial.
    inversion H10. subst b1' delta1.
    rewrite as_inj_alloc_left_sm_other in H11; trivial. 
    destruct (eq_block b2 b2'); auto. subst b2'. right; red; intros.
    eapply H6; eauto. omega.
  
  rewrite as_inj_alloc_left_sm_same in H11; trivial. 
    rewrite as_inj_alloc_left_sm_other in H10; trivial.
...
(* representable *)
  ...
  destruct (eq_block b b1).
   subst. 
    rewrite as_inj_alloc_left_sm_same in H9; trivial.
...
(* image of b1 *)
  split. unfold mu'; simpl.
    rewrite as_inj_alloc_left_sm_same; trivial.
(* image of others *)
...
(* REACH_closed*)
  red; intros.
  unfold vis, mu'. simpl.
  destruct (eq_block b b1); try subst b. trivial.
  simpl.
  apply RC. rewrite REACHAX in H9. 
  destruct H9 as [L HL].
  generalize dependent b.
  induction L; simpl; intros; inv HL.
    apply REACH_nil.
      unfold vis, mu' in H9. simpl in H9.
    ...
    specialize (IHL _ H11 n1); clear H11.
      ...
      destruct (Mem.alloc_unchanged_on (fun bb zz => True)
         _ _ _ _ _ H0) as [UP UC].
      rewrite UC in H14; trivial. 
        eapply REACH_cons; eassumption. 
Qed.
...
induction EFFSTEP; simpl in *.
  ...
  (*store*)
      ...
      apply StoreEffectD in H15. destruct H15 as [off [X Hoff]]; subst.
      clear cstepPlus H12. 
      destruct vaddr; inv H1. inv H3.
      destruct (restrictD_Some _ _ _ _ _ H20); clear H20.
      rename b0 into bSrc. rename b into bTgt.
      exists bSrc, delta.
