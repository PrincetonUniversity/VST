...
Require Import compcert.common.Memory. Import Mem.
...
Definition effect_spec := 
  external_function -> list val -> mem -> (block -> Z -> bool).

Section effect_safety.
  Context {G C Z : Type}.
  Context (Hcore : @EffectSem G C).
  Variable Hspec : ext_spec Z.
  Variable Espec : effect_spec.

  Variable ge : G.

  Fixpoint safeN (n:nat) (z:Z) (c:C) (m:mem) : Prop :=
    match n with
    | O => True
    | S n' => 
       ...
       | Some (e,sig,args), None =>
           exists x:ext_spec_type Hspec e,
             ...
             (forall ret m' z',
               ...
               Mem.unchanged_on (fun b ofs => Espec e args m b ofs = false) m m' -> 
               exists c',
