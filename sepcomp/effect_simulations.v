    ...
    core_after_external:
      ...
      forall j'
        ...
        (*hence inject_separated (as_inj mu) j' m1 m2 holds*)
         ...
         mem_forward m1 m1' -> 
         Mem.unchanged_on (fun b ofs => myBlocksSrc mu b /\ loc_unmapped (pub_of mu) b ofs) m1 m1' ->
(*think about this later         Mem.unchanged_on (loc_unmapped j) m1 m1' ->
         ...
         mem_forward m2 m2' -> 
         Mem.unchanged_on (fun b ofs => myBlocksTgt mu b /\ loc_out_of_reach (pub_of mu) m1 b ofs) m2 m2' ->
(*         Mem.unchanged_on (fun b ofs => myBlocksTgt mu b /\ loc_out_of_reach (local_of mu) m1 b ofs) m2 m2' ->*)
(*think about this later         Mem.unchanged_on (loc_out_of_reach j m1) m2 m2' ->
    ...
    eff_after_external_ModEffects: 
      forall cd mu st1 st2 m1 e vals1 m2 ef_sig vals2 e' ef_sig'
        ...
        (*val_valid/mem_wd deprecated: 
          (ValidSourceArgs: forall v1, In v1 vals1 -> val_valid v1 m1)*)
      ...
      forall nu' ret1 m1' ret2 m2'
        ...
        (*val_valid/mem_wd deprecated: 
          (RValidSrc: val_valid ret1 m1') (RValidTgt: val_valid ret2 m2')*)
     ...
     forall M1 M2
         ...
         (UnchTgt: Mem.unchanged_on (fun b ofs => ~M2 b ofs) m2 m2')
         (PrivSrcNoeffect: forall b ofs, myBlocksSrc nu b = true -> 
                           loc_unmapped (pub_of nu) b ofs -> ~M1 b ofs)

         (LocalTgtReverseEffect: forall b ofs, myBlocksTgt nu b = true -> M2 b ofs -> 
                exists b1 delta, (pub_of nu) b1 = Some(b,delta) /\ M1 b1 (ofs-delta)),

         (*This is not transitive, it seems, and neither is the variant with additional
              hypothesis RNG mu b:
         (forall b ofs, ~myBlocksTgt mu b -> M2 b ofs -> 
                 ((exists b1, delta, foreign_of mu b1 = Some(b,delta) /\ M1 b1 (ofs-delta)) 
                  \/ (forall b1 delta, foreign_of mu b1 = Some(b,delta) -> 
                      ~Mem.perm m1 b1 (ofs-delta) Max Nonempty)) ->*)
(*
         (forall b ofs, M1 b ofs -> Mem.valid_block m1 b -> Mem.perm m1 b ofs Max Nonempty) ->
         (forall b ofs, M2 b ofs -> Mem.valid_block m2 b -> Mem.perm m2 b ofs Max Nonempty) ->
*)
        exists cd', exists st1', exists st2',
          after_external Sem1 (Some ret1) st1 = Some st1' /\
          after_external Sem2 (Some ret2) st2 = Some st2' /\
    ...
    eff_after_external:
      forall cd mu st1 st2 m1 e vals1  m2 ef_sig
        (*standard assumptions:*)
        (MemInjMu: Mem.inject (as_inj mu) m1 m2)
        (MatchMu: match_state cd mu st1 m1 st2 m2)
        (AtExtSrc: at_external Sem1 st1 = Some (e,ef_sig,vals1))
...
(*
        RetBlocks 
        (RetBlocksHyp: RetBlocks = (fun b => exists z, In (Vptr b z) (ret1::nil)))

        CapturedBlocks
        (CapturedBlocksHyp: CapturedBlocks =
            leakedBlock m1' (fun b => RetBlocks b \/ exists b2 d, shared_of nu' b = Some(b2, d)))
        frgnSrc' (frgnSrcHyp: forall b, frgnSrc' b = true <-> (CapturedBlocks b /\ myBlocksSrc nu' b = false))
        frgnTgt' (frgnTgtHyp: forall b2, frgnTgt' b2 = true <-> 
                            (frgnBlocksTgt nu' b2 = true \/ exists b1 d, frgnSrc' b1 = true /\
                                                                         extern_of nu' b1 = Some(b2,d)))*)
 ...
 (*refine later:
     forall M1 M2,
         Mem.unchanged_on (fun b ofs => ~M1 b ofs) m1 m1' ->

         Mem.unchanged_on (fun b ofs => ~ M2 b ofs) m2 m2' ->

         (forall b ofs, myBlocksSrc mu b -> loc_unmapped pub' b ofs -> ~M1 b ofs) ->
         (forall b ofs, myBlocksTgt mu b -> M2 b ofs -> exists b1 delta,
                         pub' b1 = Some(b,delta) /\ M1 b1 (ofs-delta)) ->
