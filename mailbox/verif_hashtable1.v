...
Definition integer_hash_spec :=
 DECLARE _integer_hash
  ...
  POST [ tint ]
   EX i' : Z, PROP () LOCAL (temp ret_temp (vint i')) SEP ().
...
Definition wf_hists h l := Forall (fun x => (ordered_hist (fst x) /\ Forall int_op (map snd (fst x))) /\
  (ordered_hist (snd x) /\ Forall int_op (map snd (snd x)))) h /\ 0 <= l <= Zlength h /\
...
Proof.
  ...
  { exploit (Znth_inbounds i h' ([], [])).
    { destruct (Znth i h' ([], [])); intro X; inv X.
      apply app_cons_not_nil in Hi2; auto. }
    intro.
  ...
  - split.
    + rewrite Hh'; clear Hh'; rewrite Forall_app; split; [|constructor].
      * rewrite Forall_forall; intros (?, ?) Hin.
        exploit (Forall2_In_r (failed_CAS k)); eauto.
        intros ((?, ?) & Hin' & ? & ? & ? & Hcase & ? & ? & ? & ?); simpl in *; subst.
        apply sublist_In in Hin'; rewrite Forall_forall in Hwf; destruct (Hwf _ Hin') as ((? & ?) & ?).
        split; auto.
        destruct Hcase as [? | (? & ? & ?)]; subst; rewrite map_app, Forall_app; repeat constructor; auto.
  ...
  - clear Hh'; match goal with H : 0 <= l <= _ |- _ => destruct H end.
    ...
    { rewrite Forall2_eq_upto with (d1 := ([] : hist, [] : hist))(d2 := ([] : hist, [] : hist)).
      ...
      rewrite Heq2; split; auto.
      destruct Heq1 as [-> | (? & ? & ->)].
      + unfold value_of_hist at 2; rewrite last_snoc; auto.
      + rewrite app_cons_assoc; unfold value_of_hist at 2; rewrite last_snoc; simpl.
        destruct (eq_dec (Vint r1) (vint 0)); [absurd (r1 = Int.zero); auto; inv e|]; auto. }
  ...
  - unfold set.
    ...
    { rewrite Forall_forall; intros (hk', hv') Hin.
      ...
      intros (? & ? & ? & r1 & ? & Heqi & ? & ? & ? & ?); simpl in *; subst.
      destruct Heqi as [-> | (? & ? & ->)].
      + unfold value_of_hist; rewrite last_snoc; simpl.
        intro; absurd (r1 = Int.repr k); auto; apply signed_inj; auto.
        rewrite Int.signed_repr; auto.
      + rewrite app_cons_assoc; unfold value_of_hist; rewrite last_snoc; simpl.
    ...
    destruct (Znth i h' ([], [])) eqn: Hhi'; simpl in *.
    rewrite Hk'; subst; simpl.
    unfold value_of_hist; rewrite last_snoc; simpl.
    rewrite !Int.signed_repr; auto.
...
Proof.
  ...
  eapply semax_pre with (P' := EX i : Z, EX i1 : Z, EX h' : list (hist * hist),
    PROP (i1 mod 32 = (i' + i) mod 32; Forall2 (failed_CAS key) (sublist 0 i h) (sublist 0 i h');
    ...
    SEP (data_at sh (tarray (tptr tentry) 20) entries p; fold_right sepcon emp (map (atomic_entry sh) entries);
         entry_hists entries h')).
  { Exists 0 i' h; rewrite sublist_nil; entailer!. }
  eapply semax_loop.
