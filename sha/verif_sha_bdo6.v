...
semax
  ...
  (PROP  ()
   ...
   `(array_at tuint Tsh
       (upd (Xarray b (Z.of_nat i)) (Z.of_nat i mod 16)
          (Vint
             (Int.add (nthB b i 0)
                (Int.add
                   (Int.add (sigma_0 (nthB b i 1)) (sigma_1 (nthB b i 14)))
                   (nthB b i 9))))) 0 LBLOCKz)
...
clear Delta.
apply rearrange_regs2c_proof; assumption.
Qed.
...
semax Delta_loop1
  (PROP  ()
   ...
   SEP  (K_vector;
   `(array_at tuint Tsh (Xarray b (Z.of_nat i)) 0 LBLOCKz)
     (eval_var _X (tarray tuint LBLOCKz)))) bdo_loop2_body
  (normal_ret_assert
     (EX  i0 : nat,
      PROP  (LBLOCK <= i0 <= c64)
      LOCAL  (`(eq ctx) (eval_id _ctx);
...
apply (assert_LOCAL (`(eq (Vint (sigma_1 (nthB b i 14)))) (eval_id _s1))).
entailer; apply prop_right. 
rewrite add_repr in H3.
rewrite X_subscript_aux2 in H3 by repable_signed.
rewrite extract_from_b in H3 by omega.
...
Definition loop2_inv (rg0: list int) (b: list int) ctx  (delta: Z) (i: nat) :=
    ...
    LOCAL  (`(eq ctx) (eval_id _ctx); 
     ...
     `(eq (map Vint (rnd_64 rg0 K (firstn i ((rev(generate_word (rev b) c48)))))))
      (`cons (eval_id _a)
         (`cons (eval_id _b)
            (`cons (eval_id _c)
               (`cons (eval_id _d)
