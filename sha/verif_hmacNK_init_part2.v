...
(*    rewrite data_at_isptr with (p:=ctxkey). normalize.
  ...
  (PROP  ()
   ...
   `(data_at Tsh (tarray tuchar 64)
       (map Vint (map Int.repr (HP.HMAC_SHA256.mkKey key))) ctxkey);
 ...
 myframe_SEP'' [0; 3; 4].
    ...
    { assert (FR: Frame = nil).
      ...
      destruct (nth_mapVintZ i 
           (HP.HMAC_SHA256.mkArgZ (map Byte.repr (HP.HMAC_SHA256.mkKey key)) HP.Ipad)) as [n Hn].
        rewrite ZLI; assumption.
      rewrite Hn. unfold HP.HMAC_SHA256.mkArgZ in Hn; rewrite Hn. trivial.*)
    }
    after_call. simpl. intros rho. subst WITNESS. rewrite firstn_precise. normalize.
      ...
      unfold HP.HMAC_SHA256.sixtyfour. rewrite combine_length, map_length, length_list_repeat, mkKey_length.
      unfold HP.SHA256.BlockSize; simpl. trivial.
...
(*    make_sequential.
    ...
    { assert (FR: Frame = [`(sha256state_ ipadSHAabs (Vptr cb cIoff));
        `(field_at Tsh t_struct_hmac_ctx_st [StructField _md_ctx] emptySha (Vptr cb cofs));
       `(data_at Tsh (tarray tuchar 64) (map Vint (map Int.repr (HP.HMAC_SHA256.mkKey key))) ctxkey);
       `(data_at Tsh (tarray tuchar (Zlength key)) (map Vint (map Int.repr key)) (Vptr kb kofs))]).
(*         `(sha256state_ ipadSHAabs (Vptr cb (Int.add cofs (Int.repr 108))));
         `(field_at Tsh t_struct_hmac_ctx_st [_md_ctx] emptySha (Vptr cb cofs));
         `(data_at Tsh (tarray tuchar 64)
            (map Vint (map Int.repr (HP.HMAC_SHA256.mkKey key))) (Vptr ckb ckofs));
