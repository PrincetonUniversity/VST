...
(*    rewrite data_at_isptr with (p:=ctxkey). normalize.
    ...
    eapply semax_seq'. (* USING forward_seq here introduces another Delta0 in goal2 - but it worked fin before I split this off into file XXX_part2.v *)
    instantiate (1:= 
  (PROP  ()
...
(*        eapply semax_pre0; [ apply now_later | ].
  ...
  (PROP  (isptr
   ...
   `(data_at Tsh (tarray tuchar (Zlength key)) (map Vint (map Int.repr key))
       (Vptr kb kofs))))).
    { rewrite (data_at_isptr Tsh (nested_field_type2 t_struct_hmac_ctx_st [StructField _o_ctx])).  entailer. }
