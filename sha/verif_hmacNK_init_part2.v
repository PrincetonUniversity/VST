...
forward_if PostResetBranch. 
  { (* THEN*)
    ...
    { clear POSTCONDITION HeqPostResetBranch PostResetBranch.
      ...
      (*integer, ie key==NULL*)
          simpl in TC. subst i. simpl. if_tac. subst r. inversion r_true. Time entailer. (*1.7*)
      (*key == Vptr*)
       if_tac. subst r. inversion r_true. Time normalize; cancel. (*4.7*)
    }
    rewrite data_at_isptr with (p:=k). Time normalize. (*5*)
    ...
    Time assert_PROP (field_compatible t_struct_hmac_ctx_st [] (Vptr cb cofs)) as FC_C 
       by entailer. (*5.8*)
    unfold data_at at 1. Time unfold_field_at 1%nat. (*10*)
    gather_SEP 1 2 4 6.
    ...
    Time rewrite KHMS at 2. (*10secs; SLOW; Note: don't rewrite in the FRAME1-term*)
    Time forward_call (Vptr cb (Int.add cofs (Int.repr 108))). (*11.4*)
...
Definition FRAME3 (kb cb ckb: block) kofs cofs ckoff key ipadSHAabs (HMS' : reptype t_struct_hmac_ctx_st):= 
    ...
    Time forward_call (Vptr cb (Int.add cofs (Int.repr 216))). (*11*)
    ...
    subst PostResetBranch. Time entailer!. (*5.7*)
    unfold FRAME3, sha256state_; simpl.
    Time normalize. (*7.4*)
    ...
    Time cancel. (*12*)
        ...
        rewrite ZLO. Time normalize. (*6*) Time cancel. (*9*)
  ...
  { (*ELSE*) 
    Time forward. (*0.2*)
    subst. unfold initPostKeyNullConditional. Time entailer!. (*6*)
    destruct R; subst. 2: discriminate.
    simpl; clear H. Time destruct key'; try solve[entailer]. (*9.9*)
    unfold hmacstate_PreInitNull, hmac_relate_PreInitNull; simpl.
    Time destruct h1; entailer. (*7*)
    Time if_tac; [ | entailer!]. (*1.5*)
    Intros v x.
