...
Definition field_at (sh: Share.t) (t: type) (gfs: list gfield) (v: reptype (nested_field_type2 t gfs)) (p: val): mpred :=
 !! (field_compatible t gfs p /\ value_fits (nested_field_type2 t gfs) v) 
...
Definition array_at (sh: Share.t) (t: type) (gfs: list gfield) (lo hi: Z)
  ...
  !! (field_compatible0 t (ArraySubsc lo :: gfs) p /\
      ...
      Forall (value_fits (nested_field_type2 t (ArraySubsc 0 :: gfs))) v)
...
Lemma field_at_local_facts:
  forall sh t path v c,
     field_at sh t path v c |-- !! (field_compatible t path c /\ value_fits (nested_field_type2 t path) v).
Proof.
  intros.
  unfold field_at.
...
Lemma data_at_data_at': forall sh t v p,
  data_at sh t v p = !! (field_compatible t nil p /\  value_fits (nested_field_type2 t nil) v) && data_at' sh t v p.
Proof.
  intros.
...
Proof.
  ...
  + apply ND_prop_ext.
    assert (value_fits (field_type i (co_members (get_co id)))
     (proj_struct i (co_members (get_co id)) (unfold_reptype v1) d0) <-> value_fits (nested_field_type2 t (gfs DOT i))
  ...
  + apply ND_prop_ext.
    assert (value_fits (field_type i (co_members (get_co id)))
     (proj_union i (co_members (get_co id)) (unfold_reptype v1) d0) <->
       value_fits (nested_field_type2 t (gfs UDOT i))
...
Lemma nonreadable_field_at_eq {cs: compspecs} :
  forall sh t gfs v v' p,
   ~ readable_share sh ->
   (value_fits (nested_field_type2 t gfs) v <-> value_fits (nested_field_type2 t gfs) v') ->
   field_at sh t gfs v p = field_at sh t gfs v' p.
Proof.
intros.
rewrite !field_at_data_at.
...
Lemma mapsto_field_at {cs: compspecs} sh t gfs v v' p:
  ...
  type_is_volatile (nested_field_type2 t gfs) = false ->
  field_compatible t gfs p ->
  tc_val' (nested_field_type2 t gfs) v ->
  JMeq v v' ->
  mapsto sh (nested_field_type2 t gfs) (field_address t gfs p) v = field_at sh t gfs v' p.
Proof.
  intros.
...
Proof.
  ...
  erewrite (data_at_type_changable sh 
            (nested_field_array_type (Tarray tuchar n noattr) nil 0 n1)
            (Tarray tuchar n1 noattr) _ (sublist 0 n1 v)). 
  2: unfold nested_field_array_type; simpl; rewrite Zminus_0_r; trivial.
  2: apply JMeq_refl.
  erewrite (data_at_type_changable sh 
            (nested_field_array_type (Tarray tuchar n noattr) nil n1 n)
...
Lemma split2_data_at_Tarray_at_tuchar_unfold_with_fc {cs} sh n n1 v p: 0 <= n1 <= n ->
  @data_at cs sh (Tarray tuchar n noattr) v p |--
  !!(field_compatible (Tarray tuchar n noattr) nil p) &&
  (@data_at cs sh (Tarray tuchar n1 noattr) (sublist 0 n1 v) p *
   at_offset (@data_at cs sh (Tarray tuchar (n - n1) noattr) (sublist n1 (Zlength v) v)) n1 p)%logic.
Proof.
  intros. apply andp_right. 
     eapply derives_trans. apply data_array_at_local_facts. normalize.
...
Proof. intros.
  ...
  erewrite (field_at_Tarray sh  (Tarray tuchar n noattr) _ tuchar); try reflexivity; trivial. 2: omega.
  rewrite (split2_array_at sh (Tarray tuchar n noattr) nil 0 n1); trivial.
  do 2 rewrite array_at_data_at. normalize. unfold at_offset. simpl.
  rewrite Zminus_0_r.
  apply andp_right. apply prop_right.
...
Proof. intros.
  assert_PROP (Zlength v = Z.max 0 n).
    eapply derives_trans. apply data_array_at_local_facts. normalize.
  rewrite Z.max_r in H2. 2: omega. 
  eapply derives_trans.
...
|--
  @data_at cs sh (Tarray tuchar n noattr) v p.
Proof. intros. subst n.
  assert_PROP (isptr p). rewrite data_at_isptr. normalize.
  rename H1 into Pp.
  ...
  red. simpl. red in H3. simpl in *.
  repeat rewrite Z.max_r; try omega.
  rewrite Z.max_r in H3; try omega.
  assert (A: match Zlength v - lo with
   | 0 => 0
   | Z.pos y' => Z.pos y'
   | Z.neg y' => Z.neg y'
   end = Zlength v - lo). destruct (Zlength v - lo); trivial.
  rewrite A in *; clear A.
  assert (A:match Zlength v with
     | 0 => 0
     | Z.pos y' => Z.pos y'
     | Z.neg y' => Z.neg y'
     end = Zlength v). destruct (Zlength v); trivial.
  rewrite A in *; clear A.
  intuition.
...
Lemma append_split3_data_at_Tarray_at_tuchar' {cs} sh data data1 data2 data3 p:
   ...
   field_compatible (Tarray tuchar (Zlength data) noattr) nil p ->
