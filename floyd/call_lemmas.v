...
Lemma semax_call': forall Espec Delta A (Pre Post: A -> environ->mpred) (x: A) ret argsig retsig a bl P Q R,
  ...
  @semax Espec Delta
          (tc_expr Delta a && tc_exprlist Delta (argtypes argsig) bl 
           && PROPx P (LOCALx Q
            (SEPx (`(Pre x) ( (make_args' (argsig,retsig) (eval_exprlist Delta (argtypes argsig) bl))) ::
                      `(func_ptr' (mk_funspec (argsig,retsig) A Pre Post)) (eval_expr Delta a) :: R))))
          (Scall ret a bl)
          (normal_ret_assert 
            (EX old:val, 
...
Lemma semax_call0: forall Espec Delta A (Pre Post: A -> environ->mpred) (x: A) 
  ...
  @semax Espec Delta
         (tc_expr Delta a && tc_exprlist Delta (argtypes argsig) bl 
           && PROPx P (LOCALx Q
            (SEPx (`(Pre x) ( (make_args' (argsig,retty) (eval_exprlist Delta (argtypes argsig) bl))) ::
                      `(func_ptr' (mk_funspec (argsig,retty) A Pre Post)) (eval_expr Delta a) :: R))))
          (Scall None a bl)
          (normal_ret_assert 
      ...
      forall id f TC
       ...
       @semax Espec Delta 
        ...
        (SEPx (`(func_ptr' f) (eval_var id (type_of_funspec f)) :: R))))
                              c PostCond ->
       @semax Espec Delta (TC && PROPx P (LOCALx Q (SEPx R))) c PostCond.
Proof.
intros. 
...
Lemma semax_call_id0:
 forall Espec Delta P Q R id bl argsig retty A x Pre Post
  ...
  @semax Espec Delta (tc_exprlist Delta (argtypes argsig) bl
                  && PROPx P (LOCALx Q
...
eapply semax_pre_simple; [ | apply (@semax_call0 Espec Delta A Pre Post x argsig _ _ bl P Q R)].
apply andp_right.
apply andp_right.
apply andp_left1.
...
Lemma semax_call_id1:
 forall Espec Delta P Q R ret id retty bl argsig A x Pre Post
  ...
  @semax Espec Delta (tc_exprlist Delta (argtypes argsig) bl && 
                PROPx P (LOCALx Q
                 (SEPx (`(Pre x) (make_args' (argsig,Tvoid) (eval_exprlist Delta (argtypes argsig) bl)) :: R))))
    (Scall (Some ret)
...
Lemma semax_call_id0_alt:
 forall Espec Delta P Q R id bl argsig paramty retty A witness Frame Pre Post
  ...
  PROPx nil (LOCALx nil 
     (SEPx (`(Pre witness) (make_args' (argsig, retty) (eval_exprlist Delta (argtypes argsig) bl)) :: Frame))) ->
...
Lemma semax_call_id1_wow:
 forall  {A} (witness: A) (Frame: list mpred) 
   ...
   (MSUBST: force_list (map (msubst_eval_expr Delta Qtemp Qvar) 
               ...
               (fun r : environ -> mpred =>
                `r
                  (make_args' (argsig, Tvoid)
                     (eval_exprlist Delta (argtypes argsig) bl)))
...
apply andp_left2. apply andp_left1.
 assert (LEN': length (var_names argsig) = length (eval_exprlist Delta (argtypes argsig) bl rho)).
 clear - LEN.
 ...
 cut (local (fold_right `and `True (LocalD Qtemp Qvar nil)) rho |-- `(local (fold_right `and `True Qpre))
               (fun rho => (make_args (var_names argsig) (eval_exprlist Delta tys bl rho) rho)) rho).
 ...
 forget (eval_exprlist Delta tys bl rho) as vl.
