...
destruct (PTree.tree'_not_empty m2) as [i [y ?]].
specialize (H i). rewrite H. unfold PTree.get. rewrite H0. apply I.
destruct (PTree.tree'_not_empty m1) as [i [y ?]].
specialize (H i). rewrite <- H. unfold PTree.get. rewrite H0. apply I.
unfold PTree.get in H.
...
destruct (PTree.tree'_not_empty m2_2) as [i [y H0]];
 specialize (H (xI i)); simpl in H; rewrite H0 in H; simpl in H;
 rewrite ?H; auto; rewrite <- H; auto.
destruct (PTree.tree'_not_empty m1_1) as [i [y H0]];
 specialize (H (xO i)); simpl in H; rewrite H0 in H; simpl in H;
 rewrite ?H; auto; rewrite <- H; auto.
destruct (PTree.tree'_not_empty m1_2) as [i [y H0]];
 specialize (H (xI i)); simpl in H; rewrite H0 in H; simpl in H;
 rewrite ?H; auto; rewrite <- H; auto.
split.
