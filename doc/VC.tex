...
\newcommand\vstversion{2.11}
\newcommand\coqversion{8.15}
\newcommand\compcertversion{3.11}
...
\end{lstlisting}
you can use the \lstinline{rep_lia} tactic (see \autoref{refcard:rep-lia}),
...
The \lstinline{entailer}, \lstinline{entailer!}, 
and \lstinline{entailer!!} tactics
...
(see \autoref{refcard:go-lower})---but \lstinline{entailer!!}
uses a streamlined version of go-lower, see below.

Then: \lstinline{saturate_local} (see \autoref{refcard:saturate-local})---but
\lstinline{entailer!!} leaves this out, see below.
...
\paragraph{entailer!!}  The \lstinline{entailer!} tactic extracts
propositional information from \LOCAL{} and \SEP{} clauses,
and puts it above the line.  Examples: from \lstinline{LOCAL(lvar $i$ $t$ $p$)}
or \lstinline{SEP(data_at sh $t$ $v$ $p$)},
\lstinline{entailer!} asserts that $p$ is a pointer \lstinline{(isptr p)}.
If you don't need all this extra information above the line when
proving your goal, use \lstinline{entailer!!} instead.
