...
\vskip50pt
  \fontsize{14}{16}\selectfont\par\noindent{\itshape Version 1.9 \\ \today}
\vskip100pt
\par
\vskip10pt
...
In addition to parsing, \lstinline{clightgen} performs the following
transformations (to ease
...
\item $\mathbf{if}\,e\,\textbf{then}\,s_1\,\textbf{else}\,s_2;~s_3\ldots$
  \newline
  Use \lstinline{forward_if $Q$}, where $Q$ is
...
\item the loop invariant (and negation of the loop condition) is a good
...
The function-spec for \lstinline{main} is,
\begin{lstlisting}
...
\lstinline{name} tactic in the proof script above,\label{refcard:name}
...
In this case, the \lstinline{forward} tactic is easily able to prove
...
\end{lstlisting}
\vspace{-\baselineskip}
to ensure that \lstinline{size} and each element of \lstinline{contents}
are in range.
...
SEP(data_at Ews tint (Vint (Int.repr 2)) $p$;
...
\item[\textsf{name} $i$ $\_i$]
  (tactic; \autopageref{refcard:name})
