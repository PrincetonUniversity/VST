...
\vskip50pt
  \fontsize{14}{16}\selectfont\par\noindent{\itshape Version 1.8 \\ \today}
\vskip100pt
\par
\vskip10pt
...
\begin{description}
\item[Coq 8.6 or 8.6.1,] from coq.inria.fr.  Follow the standard installation instructions.
\item[CompCert 2.7.2,] from \href{https://github.com/ildyria/CompCert/tree/v2.7.2}{https://github.com/ildyria/CompCert/tree/v2.7.2}. (This is an unofficial release, since no official release of CompCert 2.7 is ported to Coq8.6.)
...
\item[VST 1.9,] from \href{vst.cs.princeton.edu}{vst.cs.princeton.edu},
...
\ychapter{Verifiable C and \textsf{clightgen}}{See PLCC Chapter \ref{ch:compcert-intro}}
...
In addition to parsing, \lstinline{clightgen} perfors the following
transformations on the program (to ease
reasoning about the program in separation logic):
\begin{itemize}
...
\autoref{with-library} describes \lstinline{wi$$th_library}.
...
\chapter{If, While, For, call}
\label{refcard:if-while-for}

To do forward proof through if-statements, while-loops, and for-loops,
you need to provide additional information: join-postconditions, loop invariants, etc.
If the first statement (not counting \lstinline{Ssequence})
in the current proof goal is \lstinline{Sifthenelse},
\lstinline{Swhile},
\lstinline{Sfor},
\lstinline{Sloop},
\lstinline{Scall}, or
you can simply apply the \lstinline{forward} tactic:
it will fail, with an error message explaining 
what you need to provide; that is, it suggests the appropriate
choice from this menu:

\begin{itemize}
\item $\mathbf{if}\,e\,\textbf{then}\,s_1\,\textbf{else}\,s_2;~s_3\ldots$
  \newline
  you do, \lstinline{forward_if $Q$}, where $Q$ is the precondition of
  statement $s_3$.  We call it the \emph{join postcondition}, as it's the
  assertion that goes where $s_1$ and $s_2$ join back together.

\item $\mathbf{if}\,e\,\mathbf{then}\,s_1\,\mathbf{else}\,s_2;\}\ldots$
  When the if-statement appears at the end of a basic block, so the
  ...
  unification variables.  You can do
  \lstinline{unfold abbreviate in POSTCONDITION} to see what's there.
...
\item $\mathbf{while}\,(e)\,s; \ldots$ \qquad (with \lstinline{break} statements in $s$)\newline
  You must treat this as if it were $\mathbf{for}\,(;e;)~ s$, and use the
  \lstinline{forward_for} tactic; see below.

\item $\mathbf{for}\,(e_1;i<e_2;i++)~ s$ \qquad (no \lstinline{break} statements in $s$)\newline
  If $e_2$ is loop-invariant, you may be able to use
  \lstinline{forward_for_simple_bound}; see \autoref{refcard:for}.

\item $\mathbf{for}\,(e_1;e_2;e_3)~ s$ \qquad (no \lstinline{break} statements in $s$)\newline
  Use \lstinline{forward_for $Q$ $Q'$}, where $Q$ is the loop invariant
  and $Q'$ is the assertion that goes right before the \emph{increment}
  command, $e_3$. See \autoref{refcard:for-general}.

\item $\mathbf{for}\,(e_1;e_2;e_3)~ s; \ldots $\newline
  If $s$ contains \lstinline{break} statements \emph{and} there are more
  commands after the loop, you will need to write
  \lstinline{forward_for $Q$ $Q'$ $R$}, where $Q$ and $Q'$ are as above,
  and $R$ is the join-postcondition. See \autoref{refcard:for-general}.

\item $f(e_1,\ldots,e_n)$ \newline
  Use \lstinline{forward_call}; see \autoref{refcard:forward-call1}.
...
to the \emph{values} of tempvars (nonaddressable
locals) and to the \emph{addresses} of 
globals and addressable locals.
A \emph{lifted predicate} of type
...
If we were to try \lstinline{forward} right now, it would fail with
a message, ``Please make sure omega or auto can prove
$(0 \le i < \mathsf{Zlength}\,\mathit{contents})$.''  That's what
would ensure that the subscript $i$ is within the bounds of the array.
...
\end{lstlisting}
The reason this is so easily provable is that
\lstinline{data_at $sh$ (tarray $\tau$ $n$) $\sigma$ $a$}
holds only when $\mathsf{Zlength}(\sigma)=n$.

Now that we have the assertion
\lstinline{H_2: Zlength contents = size} above the line,
...
ENTAIL Delta,
  ...
  LOCAL  (temp _i (Vint (Int.add (Int.repr $i$) (Int.repr 1)));
    ...
    temp _x (Znth $i$ (map Vint (map Int.repr contents)) Vundef);
    temp _a a; temp _n (Vint (Int.repr size)))
  SEP  (data_at sh (tarray tint size) (map Vint (map Int.repr contents)) a)
|-- $~$ EX  $a_0$ : Z,
    PROP  (0 <= $a_0$ <= size)
...
Vint (Int.repr (sum_Z (sublist 0 i contents) + Znth i contents 0))
\end{lstlisting}
...
\end{lstlisting}
Now the lemma \lstinline{sublist_split} is helpful here:
\begin{lstlisting}
...
to do some C-expression-evaluation, we have
\begin{lstlisting}
...
what witness would be consistent; here, we use\label{refcard:forward-call1}
...
We have discussed the \emph{lifted} separation logic, the language
of \emph{assertions} \lstinline{PROP($\ldots$) LOCAL($\ldots$) SEP($\ldots$)}
whose \lstinline{LOCAL} clause can see local variables,
and whose \lstinline{SEP} clause can see the heap.  Often we
want to hold all local variables constant and reason only about
the heap; for this we use the \emph{base level} separation logic.

Like most separation logics, it is built from predicates on ``heaplets''.
...
Require Import VST.myprogram.        (* AST of my program *)
...
Here we show a simpler proof, but see
...
Fixpoint listrep (sigma: list val) (x: val) : mpred :=
 match sigma with
 | h::hs => 
    EX y:val, data_at Tsh t_struct_list (h,y) x  * listrep hs y
 | nil => 
    !! (x = nullval) && emp
 end.
\end{lstlisting}
...
where $i$ is the value (in each iteration) of the loop iteration variable $\mathit{id}$.  You \emph{must} have an existential quantifier for the \emph{value} of the loop-iteration variable.  You \emph{may} have a second $\exists$ for a value of your choice that depends on $i$.
...
You need not write $i<\mathit{hi}$ in $P$, the tactic will
insert it.
...
\noindent\lstinline{forward_for $\mathit{Inv}$ $\mathit{PreInc}$.}\qquad or\qquad\lstinline{forward_for $\mathit{Inv}$ $\mathit{PreInc}$ $\mathit{Post}$.}
...
The \textit{Inv} and \textit{PreInc} should have type $A\rightarrow \mathrm{environ}\rightarrow\mathrm{mpred}$, where $A$ is the type of some iteration-dependent quantity (in this example, \textsf{Z}, to hold the value of loop iteration variable \textit{i}), and $\mathrm{environ}\rightarrow\mathrm{mpred}$
is the usual type of assertions.
...
Consider this example drawn from the \lstinline{main}
...
%$p$: val
\begin{lstlisting}
data_at $\mathit{sh}$ (tarray tint $k$) $\mathit{al}$ $p$ : mpred
\end{lstlisting}
\vspace{-\baselineskip}
...
Suppose we have a function \lstinline{sumarray(int a[], int n)}
...
\hspace*{1in} \lstinline{data_at $\mathit{sh}$ (tarray tint $(k-i)$) (sublist $i$ $k$ $\mathit{al}$) $q$)},\newline
...
subsub1: $a-(a-b)=b$.
Znth_list_repeat_inrange: $0 \le i \le n$ -> Znth $i$ (list_repeat (Z.to_nat $n$) a) d = a.
...
app_Znth1:  $i < \mathsf{Zlength}\, l$ -> Znth $i$ ($l$ ++ $l'$) $d$ = Znth $i$ $l$ $d$.
app_Znth2:  $i \ge \mathsf{Zlength}\, l$ -> Znth $i$ ($l$ ++ $l'$) $d$ = Znth $i-\mathsf{Zlength}\,l$ $l'$ $d$.
Znth_sublist: $0 \le i$ -> $0 \le j < k-i$ -> Znth $j$ (sublist $i$ $k$ $l$) $d$ = Znth ($j$ + $i$) $l$ $d$.
\end{lstlisting}
\vspace{-\baselineskip}
...
\emph{Every one of these needs a funspec,} of the
