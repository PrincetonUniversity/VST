...
\newthought{API spec.}
The API, or \emph{application programmer interface}, of a C program
is expressed in its header file: function prototypes and data-structure
definitions that explain how to call upon the modules' functionality.
In \emph{Verifiable C}, an \emph{API specification} is written
as a series of \emph{function specifications} (\lstinline{funspec}s)
...
DECLARE _sumarray
 ...
 PRE [ _a OF (tptr tint), _n OF tint ]
      PROP  (readable_share sh; 0 <= size <= Int.max_signed)$\label{refcard:example-readable-share}$
...
above, the statement \lstinline{0 <= size <= Int.max_signed} is
...
The C language specification says that a C compiler may treat
...
C light operational semantics. See \autoref{refcard:int-repr}.
...
In this program, \lstinline{sum_int contents} is the signed,
mod-$2^{32}$ sum.  Suppose \lstinline{$s$: list Z} is a sequence of
integers, and \lstinline{contents = map Int.repr $s$}.
Is it true that \lstinline{sum_int contents = Int.repr ($\sum s$)}?
Well, yes, but that is not necessarily what we want to know:
the more important question is: \lstinline{Int.signed (sum_int contents) $\stackrel{?}{=}~\sum s$}.
This will be true only if \lstinline{Int.min_signed $\le \sum s \le$ Int.max_signed}.
Whoever calls this \lstinline{sumarray} function will likely
need to prove this range fact about $s$ in order to make use
of the specification of the function.\footnote{What if $s$
...
But it does not need to part of the specification itself.

\newthought{In most cases,}
the WITH clause of a function-spec should not mention
values of type \lstinline{int}, but only values of type Z.
Thus, a more idiomatic use of Verifiable C would have
\lstinline{WITH$\ldots$s: list Z$\ldots$}
and then mention \lstinline{map Vint (map Int.repr s)}.
This is the way the \lstinline{size} variable is treated, for example.
...
\chapter{Integers: Z and int}

\ychapter{Values: {\upshape Vint, Vptr,} etc.}{(\file{compcert/common/Values.v})}
...
\lstinline{Vfloat($f$)} is a single-precision floating-point value.
