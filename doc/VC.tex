...
\newcommand\vstversion{2.1}
\newcommand\coqversion{8.8}
\newcommand\compcertversion{3.2}
...
Or suppose you have a lemma,
\begin{lstlisting}
Lemma L: forall x y, F(x)*G(y)=H(x,y).
\end{lstlisting}

and your proof goal is,
\begin{lstlisting}
$\mbox{\underline{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}}$
A*G(1)*C*F(2) |-- E
\end{lstlisting}
...
there is a private conjunct \lstinline{MyModuleGlobs gv}
that (presumably) uses some global variables of MyModule,
but it does not see their names.

When you do a \lstinline{semax_body} proof of a function
that \emph{directly} accesses a global variable such as \lstinline{_MyVar},
and you have \lstinline{gvars gv} in your \lstinline{LOCAL} precondition,
then \lstinline{start_function} will add
\lstinline{gvar _MyVar (gv _Myvar)} to the LOCALs clause,
so that your variable-accesses will work.

By convention, you should put \lstinline{gv:globals} as the last
component of your WITH clause.

Sometimes you may want a LOCAL assertion \lstinline{(gvar _v (gv _v))} even for a variable \lstinline{_v} that you don't use directly in this function; you can use the tactic \lstinline{assert_gvar _v} to add it (calculated from \lstinline{Delta} and \lstinline{(gvars gv)}).
