...
\newcommand\vstversion{2.4}
\newcommand\coqversion{8.9}
\newcommand\compcertversion{3.5}
...
DECLARE _sumarray
 ...
 PRE [ (tptr tuint), tint ]
    ...
    PARAMS (a; Vint (Int.repr size)) GLOBALS ()
...
\lstinline{LOCAL} propositions $\vec{Q}$ are \emph{variable
  bindings} of type \lstinline{localdef}.  In the
\lstinline{LOCAL} clause of the postcondition,
the special \lstinline{ret_temp} variable is bound to the return
value of the function.

Values of \PARAMS{} and of \LOCAL{} \lstinline{temp} variables
are C scalar variables, values whose Coq type is \lstinline{val}; this
...
(Since VST 2.6, we recommend using the new \lstinline{list_solve} tactic, described in
\autoref{refcard:list_solve} and \autoref{refcard:list_solve-advanced}.
We still provide this old version of list reasoning automation for the users that still prefer this version.)
...
\lstinline{list\_solve} supports operators:
\lstinline{Zlength},
\lstinline{Znth},
...
\lstinline{list\_solve} supports four kinds of typical proof goals:
\begin{itemize}
...
\footnote{The reason is that an element may have relationship with other elements in the same list, directly or indirectly.
...
\lstinline{rep_omega} tactic (and any that use the
\lstinline{rep_omega} hint database) can expand N.
...
The freezer abstraction consists of two main tactics, \textsf{freeze} and \textsf{thaw}.

$\mathsf{freeze}\ F\ \mathsf{:=}\ \mathit{pattern}_1  ~ \mathit{pattern}_2 \ldots \mathit{pattern}_n$\qquad
where each pattern matches \emph{one} SEP conjunct and $F$ is
 a user-supplied (fresh) Coq name.
An example pattern is \lstinline{(data_at _ $~$ _ $~$ _ $~$ p)}.
The result is that each conjunct matching a pattern is removed from the precondition's SEP clause, inserting
...
$\mathsf{thaw}\ F$ \qquad expands $F$ back into separate conjuncts.


The term $\mathit{FRZL}\ F$ participates symbolically in all
nonfreezer tactics just like any other SEP item, so can in particular
