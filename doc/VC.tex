...
% For the same reason, we use \lstinline{val} instead of \lstinline{float}
% for \lstinline{double} variables.  We do have a function
% \begin{lstlisting}
% reptype' {cs: compspecs}: type -> Type
% \end{lstlisting}
% for \emph{known initialized} values, which does use
% \lstinline{int} and \lstinline{float}; this is sometimes useful.
