...
\end{lstlisting}
\vspace{-16pt}
Also see \file{progs/verif\_min.v} for \emph{two} approaches to the specification/verification of another for-loop.
...
these two predicates.  Then the \lstinline{data_at $\ldots$ $q$)}
...
\item[\textsf{forward\_for}]
  (tactic)  This tactic does not work well in VST 1.6.  Use
  \lstinline{forward_for_simple_bound} when applicable, or
  see the method described in \autoref{refcard:for-general}.
