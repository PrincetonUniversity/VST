...
\begin{description}
\item[Coq 8.6.1,] from coq.inria.fr.  Follow the standard installation instructions.
\item[CompCert 2.7.2,] from \href{https://github.com/ildyria/CompCert}{https://github.com/ildyria/CompCert}; either unpack the zip file linked from
  \href{vst.cs.princeton.edu/download}{vst.cs.princeton.edu/download} or
  clone the repo and then do \lstinline{git checkout v2.7.2}
 (this is an unofficial release based on the official CompCert 2.7.1 for Coq 8.5, since no official release of CompCert 2.7 is ported to Coq8.6.)
  Build the \emph{clightgen} tool, using these commands:
  \textsf{./configure ia32-linux; make; make clightgen}.  You might replace
...
______________________________________(1/1)
semax Delta
 (PROP  ()  LOCAL  (gvar _four $\mathit{v\_four}$)
  SEP (data_at Ews (tarray tint 4)
     (map Vint [Int.repr 1; Int.repr 2; Int.repr 3; Int.repr 4]) $\mathit{v\_four}$))
...
$\mathit{v\_four}$.    See \autoref{refcard:gvar}.

The \lstinline{SEP} clause means that there's
data of type ``array of 4 integers'' at
address $\mathit{v\_four}$, with access permission
...
DECLARE _malloc
   WITH n:Z
   PRE [ 1%positive OF tuint ] (* parameter 1 has type unsigned int *)
