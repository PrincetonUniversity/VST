...
Lemma upd_reptype_array_length: forall t i l v, 0 <= i -> 
     (Z.to_nat i < length l)%nat -> length (upd_reptype_array t i l v) = length l.
  Proof. unfold upd_reptype_array; intros.
    rewrite app_length, force_lengthn_length_n. simpl. rewrite skipn_length.
    unfold nat_of_Z.
    rewrite minus_Sn_m. 2: rewrite Z2Nat.inj_add; simpl; omega.
    rewrite Z.add_comm, Z2Nat.inj_add; trivial. 
    assert ((Z.to_nat 1 + Z.to_nat i = S(Z.to_nat i))%nat). trivial.
    rewrite H1, NPeano.Nat.sub_succ; clear H1.
    rewrite <- le_plus_minus; trivial. omega.
  Qed. 


Lemma upd_reptype_array_lookup K t: forall l (L:length l = K%nat) i (I: (0<=i<K)%nat) j (J: (0<=j<K)%nat) d v,
   (i=j /\ nth i (upd_reptype_array t (Z.of_nat j) l v) d = v) \/
   (i<>j /\ nth i (upd_reptype_array t (Z.of_nat j) l v) d = nth i l d).
intros. unfold upd_reptype_array, nat_of_Z; simpl. repeat rewrite Z2Nat.inj_add, Nat2Z.id; try omega.
...
Lemma upd_reptype_array_lookup' K t: forall l (L:length l = K%nat) i (I: (0<=i<K)%nat) 
                                      j (J: (0<=j<Z.of_nat K)) d v,
   nth i (upd_reptype_array t j l v) d = if beq_nat i (Z.to_nat j) then v else nth i l d.
Proof. intros.
destruct (upd_reptype_array_lookup K t l L i I (Z.to_nat j)) with (d:=d)(v:=v) as [[? ?] | [? ?]]. 
  split. destruct (Z2Nat.inj_le Z0 j). omega. omega. simpl in H. apply H. omega.
         destruct (Z2Nat.inj_lt j (Z.of_nat K)). omega. omega. rewrite Nat2Z.id in H. omega.
  subst i; rewrite <- beq_nat_refl. rewrite Z2Nat.id in H0; trivial. omega.
apply beq_nat_false_iff in H. rewrite Z2Nat.id in H0. rewrite H; trivial. omega.
Qed.


Lemma upd_reptype_array_char t n l1 v l2 w: length l1=n ->
     upd_reptype_array t (Z.of_nat n) (l1 ++ v :: l2) w = l1 ++ w :: l2.
 Proof. intros. unfold upd_reptype_array, nat_of_Z. rewrite Nat2Z.id.
   f_equal. rewrite force_lengthn_app1; trivial.
   f_equal. rewrite Z2Nat.inj_add, skipn_app2; try rewrite H, Nat2Z.id; try omega.
            rewrite minus_plus; simpl. reflexivity.
 Qed.

Lemma upd_Znth_same: forall t i l u v, 0<= i< Zlength l -> Znth i (upd_reptype_array t i l u) v = u.
   ...
   destruct (upd_reptype_array_lookup _ t l (eq_refl _) _ H1 _ H1 v u); simpl.
      rewrite Z2Nat.id in H2. apply H2. omega.
      destruct H2; omega.
Qed.
Lemma upd_Znth_diff: forall t i j l u v, 0<= i< Zlength l -> 0<= j< Zlength l -> i<>j -> Znth i (upd_reptype_array t j l u) v = Znth i l v.
   ...
   destruct (upd_reptype_array_lookup _ t l (eq_refl _) _ H3 _ H4 v u).
...
Lemma upd_reptype_ints i xints v: (nat_of_Z i < length xints)%nat ->
      upd_reptype_array tuint i (map Vint xints) (Vint v) =
      map Vint (force_lengthn (nat_of_Z i) xints Int.zero ++
                v :: skipn (nat_of_Z (i + 1)) xints).
Proof. unfold upd_reptype_array; intros.
  erewrite force_lengthn_mapN.
  rewrite map_app, sha_lemmas.skipn_map. reflexivity. trivial.
Qed.
