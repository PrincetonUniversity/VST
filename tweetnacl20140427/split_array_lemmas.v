...
Proof.
  ...
  2: apply JMeq_refl.
  simpl. 
  assert (match n1 with
            | 0 => 0
            | Z.pos y' => Z.pos y'
            | Z.neg y' => Z.neg y'
            end = n1). destruct n1; trivial. rewrite H3; clear H3.
...
Proof. intros.
  ...
  assert (N1: match n1 with
            ...
            | Z.pos y' => Z.pos y'
            | Z.neg y' => Z.neg y'
            end = n1). destruct n1; trivial. rewrite N1. clear N1.
...
Proof. intros. subst n.
  ...
  unfold legal_alignas_type, nested_pred, local_legal_alignas_type in *. simpl in *.
  rewrite Zle_imp_le_bool. trivial. omega.
  rewrite Z.max_r. 2: omega. assert (N: match Zlength v - lo with
      | 0 => 0 | Z.pos y' => Z.pos y' | Z.neg y' => Z.neg y' end = Zlength v - lo). destruct (Zlength v - lo); trivial.
    rewrite N; clear N. omega.
  rewrite Z.max_r. 2: omega. assert (N: match Zlength v - lo with
      | 0 => 0 | Z.pos y' => Z.pos y' | Z.neg y' => Z.neg y' end = Zlength v - lo). destruct (Zlength v - lo); trivial.
    rewrite N; clear N.
  rewrite Z.max_r in *; try omega. assert (N: match Zlength v  with
      | 0 => 0 | Z.pos y' => Z.pos y' | Z.neg y' => Z.neg y' end = Zlength v). destruct (Zlength v); trivial.
    rewrite N in *; clear N.
