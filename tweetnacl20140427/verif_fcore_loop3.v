...
Fixpoint WcontI (xs: list int) (j:nat) (l:list int):Prop :=
   match j with O => length l = 16%nat /\l=xs
   | (S n) => length l = 16%nat /\
              exists t0 t1 t2 t3,
...
Lemma WcontI_length xs: forall j l, WcontI xs j l -> length xs=16%nat /\ length l=16%nat.
...
destruct r; simpl in H. inv H. rename i into x0.
destruct r; simpl in H. inv H. rename i into x1. 
destruct r; simpl in H. inv H. rename i into x2.
destruct r; simpl in H. inv H. rename i into x3.
destruct r; simpl in H. inv H. rename i into x4.
destruct r; simpl in H. inv H. rename i into x5.
destruct r; simpl in H. inv H. rename i into x6.
destruct r; simpl in H. inv H. rename i into x7.
destruct r; simpl in H. inv H. rename i into x8.
destruct r; simpl in H. inv H. rename i into x9.
destruct r; simpl in H. inv H. rename i into x10.
destruct r; simpl in H. inv H. rename i into x11.
destruct r; simpl in H. inv H. rename i into x12.
destruct r; simpl in H. inv H. rename i into x13.
destruct r; simpl in H. inv H. rename i into x14.
destruct r; simpl in H. inv H. rename i into x15.
...
destruct W3 as [_ W3]; simpl in W3. unfold upd_intlist in W3; simpl in W3.
...
destruct W2 as [_ W2]; simpl in W2. unfold upd_intlist in W2; simpl in W2.
...
destruct W1 as [_ W1]; simpl in W1. unfold upd_intlist in W1; simpl in W1.
     ...
     (Sfor (Sset _i (Econst_int (Int.repr 0) tint))
        ...
        (Ssequence
           (Sfor (Sset _j (Econst_int (Int.repr 0) tint))
              (Ebinop Olt (Etempvar _j tint) (Econst_int (Int.repr 4) tint)
                 tint)
                    (Ssequence
                      (Ssequence
...
Focus 2. entailer. apply (exp_right r). cancel.
         apply andp_right. apply prop_right. apply H9. cancel.
{ rename H into I.
  ...
  (PROP  ()
   ...
   `(CoreInSEP data (nonce, c, k));
   `(data_at Tsh (tarray tuchar 64) OUT out)))).
  { entailer. apply (exp_right nil). entailer. admit. (*data_at_ length: should be ok once we have data_at_ tarray enforcing correct length*) }
  { rename H into J. rename i0 into j.
    ...
    rewrite X. simpl. split. apply H12.
  ...
  assert (WL: length wlist = 16%nat) by apply WLIST.
  eapply semax_post'.
  Focus 2. apply array_copy3; trivial. rewrite map_length; trivial.
           rewrite map_length. eapply Snuffle_length. eassumption.
             assumption.
           intros. apply Znth_mapVint. rewrite Zlength_correct, WL. apply H.
  entailer.
