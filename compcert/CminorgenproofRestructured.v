...
exists T2 : state,
  plus step tge
    (State tfn (Sassign (for_temp id) x) tk (Vptr sp Int.zero) te tm) E0 T2 /\
  match_statesInj j
    (Csharpminor.State f Csharpminor.Sskip k e (PTree.set id v le) m) T2.
Proof. intros.
  exploit transl_expr_correct; eauto. intros [tv [EVAL [VINJ APP]]].
  econstructor; split.
...
exists T2 : state,
  plus step tge
    (State tfn
       (Scall (option_map for_temp optid) (Csharpminor.funsig fd) x x1) tk
...
exists T2 : state,
  plus step tge
    (State tfn (Sbuiltin (option_map for_temp optid) ef x) tk
...
(AlocVars : alloc_variables empty_env m (fn_variables f) e m1)
...
(MK : match_cont k tk (sig_res (Csharpminor.funsig (Internal f))) cenv nil cs)
(ISCC : Csharpminor.is_call_cont k)
...
exists j', inject_incr j j' /\
  match_statesInj j'
    (Csharpminor.State f (Csharpminor.fn_body f) k e empty_temp_env m2) T2.
Proof. intros.
...
(ExtCall : @external_call ef Csharpminor.fundef var_kind ge vargs m t vres m')
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm cs (Mem.nextblock m) (Mem.nextblock tm))
(MK : match_cont k tk (sig_res (Csharpminor.funsig (External ef))) cenv nil cs)
(ISCC : Csharpminor.is_call_cont k)
(ARGSINJ : val_list_inject j vargs targs),
exists T2 : state,
...
(MINJ : Mem.inject j m tm)
(MCS : match_callstack j m tm cs (Mem.nextblock m) (Mem.nextblock tm))
(MK : match_cont (Csharpminor.Kcall optid f e le k) tk ty cenv (@nil bool) cs)
...
(* skip call *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_SkipCall _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H H0 H1 TRF MINJ MCS MK) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* assign *)
  inv MSTATE. monadInv TR. rename f0 into j.
  destruct (transl_step_case_Assign _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H H0 TRF MINJ MCS MK EQ EQ0) as [T2 [Steps MSI]].
   apply matchInj_Elim in MSI. eauto.
(* set *)
  inv MSTATE. monadInv TR. rename f0 into j.
...
(* internal call *)
  inv MSTATE. monadInv TR. rename f0 into j. 
  destruct (transl_step_case_InternalCall _ _ _ _ _ _ _ _ _ _ _ _ _ _ H H0 H1 MINJ MCS MK ISCC ARGSINJ EQ) as [T2 [Steps [j' [Inc MSI]]]].
   apply matchInj_Elim in MSI. eauto.
