...
Proof with (try exact I).
  ...
  try exact I;
  try now (destruct v0; try destruct v1; try exact I).
  congruence.
*  unfold Genv.symbol_address; destruct (Genv.find_symbol genv i)...
...
Proof.
  ...
  try now (repeat match goal with
          | H: Val.inject _ ?A ?B |- _ => try clear H A B; inv H
          end; constructor).
*  apply GL; simpl; auto.
...
*  inv H4; inv H2; simpl; auto. destruct (Int.ltu i0 Int.iwordsize); auto.
*  inv H4; simpl; auto. destruct (Int.ltu i Int.iwordsize); auto.
*  inv H4; simpl; auto. destruct (Int.ltu i Int.iwordsize); auto.  inv H2; simpl; auto. destruct (Int.ltu (Int.sub Int.iwordsize i) Int.iwordsize); auto.
