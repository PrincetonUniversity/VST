...
Hypothesis eq_after_external:
   forall (d : C1) (st1 : core_data) (st2 : C2) (ret : val) (e : external_function) (args : list val) (ef_sig : signature),
    (d = st1 /\ match_cores d st1 st2) ->
    at_external Sem1 st1 = Some (e, ef_sig, args) ->
    at_external Sem2 st2 = Some (e, ef_sig, args) ->
    Forall2 Val.has_type args (sig_args ef_sig) ->
...
Lemma  eq_simulation_star_wf: Forward_simulation_eq.Forward_simulation_equals _ _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
...
Lemma  eq_simulation_star: Forward_simulation_eq.Forward_simulation_equals _ _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
...
Lemma  eq_simulation_plus: Forward_simulation_eq.Forward_simulation_equals _ _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
...
Lemma  ext_simulation_star_wf: Forward_simulation_ext.Forward_simulation_extends _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
  eapply Forward_simulation_ext.Build_Forward_simulation_extends with
   ...
   intros. destruct (match_initial_cores _ _ _ H _ _ _ _ H0 H1 H2) as [c1' [c2' [MIC1 [MIC2 MC]]]].
                 exists c1'. exists c1'. exists c2'. split; eauto.
...
Section EXT_SIMULATION_STAR.
  ...
  Hypothesis ext_star_simulation:
    forall c1 m1 c1' m1', corestep Sem1 ge1 c1 m1 c1' m1'  -> 
    forall c2 m2, match_states c1 c1 m1 c2 m2 ->
      (exists c2', exists m2', corestep_plus Sem2 ge2 c2 m2 c2' m2' /\ match_states c1' c1' m1' c2' m2')
     \/ (measure c1' < measure c1 /\ (*apparently not needed: Mem.extends m1' m2 /\ *) match_states c1' c1' m1' c2 m2)%nat.

Lemma  ext_simulation_star: Forward_simulation_ext.Forward_simulation_extends _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
  eapply ext_simulation_star_wf.
        apply  (well_founded_ltof _ measure).
        intros. destruct (ext_star_simulation _ _ _ _ H _ _ H0).
             destruct H1 as [c2' [m2' [CSP MC']]]. exists c2'. exists m2'. split; trivial. left; trivial.
             destruct H1 as [X1 X3]; subst. exists c2. exists m2. split; trivial. right. split; trivial.
             eapply corestep_star_zero. 
Qed.
...
Lemma  ext_simulation_plus: Forward_simulation_ext.Forward_simulation_extends _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
...
Lemma  inj_simulation_star: Forward_simulation_inj.Forward_simulation_inject _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
...
Lemma  inj_simulation_plus: Forward_simulation_inj.Forward_simulation_inject _ _ Sem1 Sem2 ge1 ge2 entry_points.
Proof.
