...
Definition mem_unchanged_on (P: block -> Z -> Prop) (m_before m_after: mem): Prop :=
  (forall b ofs k p, Mem.valid_block m_before b -> 
   P b ofs -> Mem.perm m_before b ofs k p -> Mem.perm m_after b ofs k p)
/\(forall chunk b ofs v, Mem.valid_block m_before b ->
...
Definition inject_separated (f f': meminj) (m m': mem): Prop :=
  forall b1 b2 delta,
  f b1 = None -> f' b1 = Some(b2, delta) ->
  ~Mem.valid_block m b1 /\ ~Mem.valid_block m' b2.

Lemma mem_unchanged_unmapped_trans : forall m1 m1' m2 m3 f1 f2,
  mem_unchanged_on (loc_unmapped f1) m1 m2 -> 
  inject_separated f1 f2 m1 m1' -> 
  (forall b, Mem.valid_block m1 b -> Mem.valid_block m2 b) ->
  mem_unchanged_on (loc_unmapped f2) m2 m3 -> 
  mem_unchanged_on (loc_unmapped f1) m1 m3.
Proof.
intros until f2; intros [UN1a UN1b] SEP1 VALID [UN2a UN2b].
split.
intros until p; intros H1 H2 H3.
specialize (UN1a _ _ _ _ H1 H2 H3).
assert (H4: loc_unmapped f2 b ofs).
 unfold loc_unmapped in H2|-*.
 case_eq (f2 b); auto.
 intros [b' delta'] H4.
 destruct (SEP1 b b' delta' H2 H4) as [H5 ?].
 elimtype False; apply H5; auto.
solve[apply (UN2a _ _ _ _ (VALID _ H1) H4 UN1a)].
intros until v; intros H1 H2 H3.
specialize (UN1b _ _ _ _ H1 H2 H3).
specialize (UN2b chunk b ofs v (VALID _ H1)).
apply UN2b; auto.
intros i H4; specialize (H2 _ H4).
unfold loc_unmapped in H2|-*.
case_eq (f2 b); auto.
intros [b' delta'] H5.
destruct (SEP1 b b' delta' H2 H5) as [H6 ?].
elimtype False; apply H6; auto.
Qed.

Lemma mem_unchanged_outofbounds_trans : forall m1 m2 m1' m2' m3',
  Mem.extends m1 m1' -> 
  Mem.extends m2 m2' -> 
  ...
  mem_unchanged_on (loc_out_of_bounds m2) m2' m3' -> 
  mem_unchanged_on (loc_out_of_bounds m1) m1' m3'.
Proof.
intros until m3'; intros EXT1 EXT2 [UN1a UN1b] VALID MAX [UN2a UN2b].
split.
intros until p; intros H1 H2 H3.
specialize (UN1a _ _ _ _ H1 H2 H3).
apply UN2a; auto.
unfold loc_out_of_bounds in H2|-*.
intros H4; apply H2; apply MAX; auto.
unfold Mem.valid_block in H1|-*.
solve[destruct EXT1 as [Heq ?]; rewrite Heq; auto].
intros until v; intros H1 H2 H3.
specialize (UN1b _ _ _ _ H1 H2 H3).
apply UN2b; auto.
intros i H4.
specialize (H2 _ H4).
unfold loc_out_of_bounds in H2|-*.
intros H5; apply H2; apply MAX; auto.
unfold Mem.valid_block in H1|-*.
solve[destruct EXT1 as [Heq ?]; rewrite Heq; auto].
Qed.

Lemma mem_unchanged_outofreach_trans : forall m1 m2 m1' m2' m3' f1 f2,
  Mem.inject f1 m1 m1' -> 
  Mem.inject f2 m2 m2' -> 
  ...
  (forall b ofs p, Mem.valid_block m1 b -> Mem.perm m2 b ofs Max p -> 
    Mem.perm m1 b ofs Max p) -> 
  inject_incr f1 f2 -> 
  mem_unchanged_on (loc_out_of_reach f2 m2) m2' m3' -> 
  mem_unchanged_on (loc_out_of_reach f1 m1) m1' m3'.
Proof.
intros until f2; intros INJ1 INJ2 [UN1a UN1b] SEP1 VALID MAX INCR [UN2a UN2b].
split.
intros until p; intros H1 H2 H3.
specialize (UN1a _ _ _ _ H1 H2 H3).
apply UN2a; auto.
...
case_eq (f1 b0).
intros [b0' delta0'] H6.
specialize (INCR _ _ _ H6).
rewrite INCR in H4.
inv H4.
specialize (H2 b0 delta0 H6).
intros H7.
apply H2.
apply MAX; auto.
assert (H8: Mem.valid_block m1 b0).
eapply Mem.valid_block_inject_1; eauto. 
auto.

intros H5 H6.
destruct (SEP1 _ _ _ H5 H4) as [H7 H8].
assert (H9: Mem.valid_block m2 b0).
 solve[eapply Mem.valid_block_inject_1; eauto].
unfold inject_separated in SEP1.
destruct (SEP1 _ _ _ H5 H4) as [H10 H11].
...
specialize (H2 _ H4).
unfold loc_out_of_reach in H2|-*.
intros b0 delta0 H5 H6.
case_eq (f1 b0).
intros [b0' delta0'] H7.
specialize (INCR _ _ _ H7).
rewrite INCR in H5.
inv H5.
apply (H2 b0 delta0 H7); auto.
apply MAX; auto.
eapply Mem.valid_block_inject_1; eauto.

intros H7.
destruct (SEP1 _ _ _ H7 H5).
auto.
Qed.
...
Proof.
  ...
  rename H into H'; rename H4 into H; rename H5 into H4.
...
Proof.
  ...
  rename H1 into Hvalid; rename H6 into H1; rename H7 into H6.
...
Proof.
  ...
  rename H2 into Hvalid; rename H3 into H2; rename H4 into H3.
...
(* extensions *)
  ...
  rename H into Hvalid; rename H1 into H; rename H10 into H1; rename H11 into H10.
...
(* injections *)
  ...
  rename H0 into Hvalid; rename H2 into H0; rename H12 into H2.
