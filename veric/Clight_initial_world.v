...
Lemma initial_core_funassert :
  forall (prog: program) V G m (*ve te*)
      ...
      (Hmatch : match_fdecs (prog_funct prog) G)
      (Hm : Genv.init_mem prog = Some m),
  initial_core m (globalenv prog) G ∗ matchfunspecs (globalenv prog) G ⊢ funassert (nofunc_tycontext V G) (globalenv prog).
Proof.
  intros; iIntros "(#H & match)"; iSplitL ""; rewrite /initial_world.initial_core /Map.get /=.
