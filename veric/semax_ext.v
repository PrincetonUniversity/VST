...
Definition funspec2post (ext_link: Strings.String.string -> ident) (A : TypeTree)
  (Q: forall ts, dependent_type_functor_rec ts (ArgsTT A) mpred)
...
Definition funspec2extspec (ext_link: Strings.String.string -> ident) (f : (ident*Newfunspec))
  : external_specification juicy_mem external_function Z :=
  match f with
    | (id, mk_Newfunspec ((params, sigret) as fsig) cc A P Q NEP NEQ) =>
...
Definition wf_funspec (f : Newfunspec) :=
  match f with
    | mk_Newfunspec sig cc A P Q _ _ =>
...
Next Obligation.
destruct f; simpl; unfold funspec2pre, pureat; simpl; destruct n(*f*); simpl;
...
Qed.
Next Obligation.
destruct f; simpl; unfold funspec2post, pureat; simpl; destruct n(*f*); simpl;
  destruct t; simpl; intros.
if_tac [e0|e0].
...
Qed.
Next Obligation.
intros ? ? ? ?; destruct f; destruct n(*f*); destruct t; simpl.
intros a' Hage; auto.
Qed.
...
Lemma add_funspecs_pre  (ext_link: Strings.String.string -> ident)
  ...
  In (ext_link id, (mk_Newfunspec sig cc A P Q NEP NEQ)) fs ->
...
destruct a; simpl; destruct n(*f*); simpl; destruct t; simpl; unfold funspec2pre; simpl.
if_tac [e|e].
...
Lemma add_funspecs_pre_void  (ext_link: Strings.String.string -> ident)
  ...
  let ef := EF_external id (mksignature (map typ_of_type sig) None cc) in
  funspecs_norepeat fs ->
  In (ext_link id, (mk_Newfunspec (sig, tvoid) cc A P Q NEP NEQ)) fs ->
  join phi0 phi1 (m_phi m) ->
  Val.has_type_list args (sig_args (ef_sig ef)) ->
  P (projT1 x) (projT2 x) (filter_genv (symb2genv ge_s), args) phi0 ->
  exists x' : ext_spec_type (JE_spec _ (add_funspecs_rec ext_link Z Espec fs)) ef,
...
destruct a; simpl; destruct n(*f*); simpl; unfold funspec2post; simpl.
destruct t; simpl.
if_tac [e|e].
* injection e as E; subst i; destruct fs; [solve[simpl; intros; elimtype False; auto]|].
  intros x' Ha Hb; simpl in Ha, Hb.
...
Lemma semax_ext' (ext_link: Strings.String.string -> ident) id sig cc A P Q NEP NEQ (fs : funspecs) :
  ...
  In (ext_link  id,f) fs ->
  funspecs_norepeat fs ->
  (forall n, semax_external (add_funspecs Espec ext_link fs) (*no idents here any longer(fst (split (fst sig)))*)
...
Lemma semax_ext'_void (ext_link: Strings.String.string -> ident) id sig cc A P Q NEP NEQ (fs : funspecs) :
  ...
  (forall n, semax_external (add_funspecs Espec ext_link fs) (*(fst (split sig))*)
               (EF_external id (mksignature (map typ_of_type (*(map snd sig)*)sig) None cc)) _ P Q n).
Proof.
intros f Hin Hnorepeat.
unfold semax_external.
intros n ge Ts x n0 Hlater F ts args jm H jm' H2 [Hargsty H3].
...
Lemma semax_ext_void (ext_link: Strings.String.string -> ident) id (*ids*) sig sig' cc A P Q NEP NEQ (fs : funspecs) :
  ...
  funspecs_norepeat fs ->
  (*ids = fst (split sig) ->*)
  sig' = mksignature (map typ_of_type (*(map snd sig)*)sig) None cc ->
  (forall n, semax_external (add_funspecs Espec ext_link fs) (*ids*) (EF_external id sig') _ P Q n).
Proof.
intros; subst.
