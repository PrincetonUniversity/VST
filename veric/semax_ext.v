...
Definition funspec2pre (ext_link: Strings.String.string -> ident) (A : TypeTree)
  ...
  (ids: list ident) (id: ident) (ef: external_function) x ge_s
...
Definition funspec2extspec (ext_link: Strings.String.string -> ident) (f : (ident*funspec))
  ...
  match f with
    | (id, mk_funspec (params, sigret) _ A P Q NEP NEQ) =>
      Build_external_specification juicy_mem external_function Z
        (fun ef => if oi_eq_dec (Some id) (ef_id ext_link ef) then (rmap* (sigT (fun ts => dependent_type_functor_rec ts A mpred)))%type else ext_spec_type Espec ef)
...
intros n ge Ts x n0 Hlater F ts args jm H jm' H2 H3.
...
{ generalize (all_funspecs_wf f) as Hwf2; intro.
  ...
  apply Hwf2; auto. }
  destruct (@add_funspecs_pre ext_link _ _ _ _ _ _ _ _ _ _ (existT _ Ts x) _ _ OK_spec ts (Genv.genv_symb ge) s t Hnorepeat Hin Hjoin Hp'')
  as [x' [Heq Hpre]].
simpl.
exists x'.
intros z.
...
{ generalize (all_funspecs_wf f) as Hwf2; intro.
  ...
  rewrite symb2genv_ax; auto.
  apply Hwf2; auto. }
destruct (@add_funspecs_pre_void ext_link _ _ _ _ _ _ _ _ _ _ (existT _ Ts x) _ _ OK_spec ts (Genv.genv_symb ge) s t Hnorepeat Hin Hjoin Hp'')
  as [x' [Heq Hpre]].
simpl.
exists x'.
intros z.
