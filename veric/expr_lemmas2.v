...
destruct (eval_expr Delta e rho); simpl; eauto.
*
...
unfold eval_field.
destruct (eval_lvalue Delta e rho); eauto;
destruct (typeof e); try congruence; auto.
destruct ((composite_types Delta) ! i0) as [co |]; auto.
destruct (field_offset (composite_types Delta) i (co_members co)); eauto.
unfold eval_field.
destruct (eval_lvalue Delta e rho); eauto;
destruct (typeof e); try congruence; auto;
destruct ((composite_types Delta) ! i0) as [co |]; auto;
...
destruct ((composite_types Delta) ! i0) as [co |]; try inv H3.
destruct (field_offset (composite_types Delta) i (co_members co)); try inv H3.
simpl; eauto.
...
destruct ((composite_types Delta) ! i0) as [co |]; intuition.
destruct (field_offset (composite_types Delta) i (co_members co)); intuition.
Qed.
...
destruct (typeof e); intuition;
destruct ((composite_types Delta) ! i1) as [co |]; intuition.
destruct (field_offset (composite_types Delta) i (co_members co)); intuition.
Qed.
...
unfold_lift.
destruct (eval_expr Delta e rho); try contradiction.
destruct pt; inv H1; reflexivity.
Qed.
