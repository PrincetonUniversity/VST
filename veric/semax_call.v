...
Lemma semax_call_aux {CS Espec}
  ...
  (HR: ( ALL rho' : environ , |>
       !((EX old:val, (*!!(tc_val' retty old) &&*)
                     substopt ret (`old) F rho' * maybe_retval (nQ ts x) retty ret rho') >=> own.bupd (RA_normal R rho') )) (m_phi jm))
...
Lemma semax_call_aux' {CS Espec}
  ...
  nQ
  (HR: ( ALL rho' : environ ,
       !((EX old:val, (*!!(tc_val' retty old) &&*) substopt ret (`old) F rho' * maybe_retval (nQ ts x) retty ret rho') >=> own.bupd (RA_normal R rho') )) (m_phi jm))
  (CSUB: cenv_sub (@cenv_cs CS) (genv_cenv psi))
  (Hrho: rho = construct_rho (filter_genv psi) vx tx)
  (EvalA: eval_expr a rho = Vptr b Ptrofs.zero)
  (Funassert: funassert Delta rho (m_phi jm))
