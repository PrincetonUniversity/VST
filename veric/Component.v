...
Definition semaxfunc_InternalInfo C V G (ge : Genv.t fundef type) id f phi := 
exists b,
...
Definition semaxfunc_ExternalInfo Espec (ge : Genv.t fundef type) (id : ident) 
...
Definition SF {Espec cs V ge} G (i:ident) (fd:fundef) (phi:funspec) :=
  match fd with
...
Lemma Comp_Imports_sub Imports' (HI1: map fst Imports' = map fst Imports)
      ...
      (HI2: Forall2 funspec_sub (map snd Imports') (map snd Imports)): @Component Espec V cs E Imports' p Exports.
Proof.
...
Lemma Comp_Exports_sub Exports' (HE1: map fst Exports' = map fst Exports)
      (HE2: Forall2 funspec_sub (map snd Exports) (map snd Exports')): @Component Espec V cs E Imports p Exports'.
Proof.
...
Variable Ge1_FS: forall i, sub_option (Genv.find_symbol (Genv.globalenv p1) i) 
                                      (Genv.find_symbol (Genv.globalenv p) i).

Variable Ge2_FS: forall i, sub_option (Genv.find_symbol (Genv.globalenv p2) i) 
                                      (Genv.find_symbol (Genv.globalenv p) i).

Variable Ge1_FFP: forall b, sub_option (Genv.find_funct_ptr (Genv.globalenv p1) b)
                                       (Genv.find_funct_ptr (Genv.globalenv p) b).

Variable Ge2_FFP: forall b, sub_option (Genv.find_funct_ptr (Genv.globalenv p2) b)
                                       (Genv.find_funct_ptr (Genv.globalenv p) b).
...
{ clear - FundefsMatch JUST1 JUST2. intros ? ? ? I1 I2; specialize (FundefsMatch i). 
                     ...
                     destruct I1 as [ [_ [? [? [? [? HH1]]]]] | [_ [ ? [? HH1]]]]; rewrite (Fundef_of_Gfun HH1) in *;
                     destruct I2 as [ [_ [? [? [? [? HH2]]]]] | [_ [ ? [? HH2]]]]; rewrite (Fundef_of_Gfun HH2) in *;
                     specialize (JUST1 _ _ (eq_refl _) (eq_refl _)); simpl in JUST1;
                     specialize (JUST2 _ _ (eq_refl _) (eq_refl _)); simpl in JUST2;
                     specialize (FundefsMatch _ _ (eq_refl _) (eq_refl _)); simpl in FundefsMatch; subst; inv FundefsMatch;
...
+ (*SF*) subst G. intros. clear Condition1 Condition2 ImportsDef. apply G_merge_find_id_Some in H0. 2: apply (Comp_G_LNR c2).
  ...
  - subst phi; 
    ...
    rewrite FD1 in Functions_preserved, FundefsMatch. red in JUST1.
    ...
    * exploit (@Comp_G_in_Fundefs' _ _ _ _ _ _ _ c2). apply Heqq2. intros [fd2 FD2].
      ...
      rewrite FD2 in Functions_preserved, FundefsMatch. red in JUST2.
      ...
      ++ (*Internal/Internal*)
         ...
         assert (SF1: semaxfunc_InternalInfo cs (*(V1 ++ V2)*)V (Imports ++ G_merge (Comp_G c1) (Comp_G c2)) (Genv.globalenv p) i f0 phi1).
         { eapply InternalInfo_subsumption.
           4:  apply (InternalInfo_envs_sub CS1 (Genv.globalenv p1)); [trivial | trivial | apply JUST1]. 
           trivial. trivial. apply Comp_ctx_LNR. }
         assert (SF2: semaxfunc_InternalInfo cs (*(V1 ++ V2)*)V (Imports ++ G_merge (Comp_G c1) (Comp_G c2)) (Genv.globalenv p) i f0 phi2).
         { eapply InternalInfo_subsumption.
           4: apply (InternalInfo_envs_sub CS2 (Genv.globalenv p2)); trivial; apply JUST2.
           trivial. trivial. apply Comp_ctx_LNR. } 
      ...
      ++ (*ExternalExternal*)
         rewrite Functions_preserved in H. inv H. inv FundefsMatch.
         assert (SF1: semaxfunc_ExternalInfo Espec (Genv.globalenv p) i e0 t1 t2 c0 phi1).
         { eapply ExternalInfo_envs_sub. 3: apply JUST1. trivial. trivial. }
         assert (SF2: semaxfunc_ExternalInfo Espec (Genv.globalenv p) i e0 t1 t2 c0 phi2).
    ...
    * (*i in G1 but not in G2*)
      ...
      ++ destruct EF1 as [ef [tys [rt [cc EF1]]]]. apply Gfun_of_Fundef in FD1; [ rewrite FD1 in EF1; inv EF1 | apply c1].
      ...
      ++ destruct IF1 as [f IF1]. apply Gfun_of_Fundef in FD1; [ rewrite FD1 in IF1; inv IF1 | apply c1].
         ...
         -- destruct f0.
            ** specialize (FundefsMatch _ _ (eq_refl _) (eq_refl _)). simpl in FundefsMatch. inv FundefsMatch. 
               ...
               assert (SF1: semaxfunc_InternalInfo cs (*(V1 ++ V2)*)V (Imports ++ G_merge (Comp_G c1) (Comp_G c2)) (Genv.globalenv p) i f0 (merge_specs phi1 None)).
               { eapply InternalInfo_subsumption.
                 4: apply (InternalInfo_envs_sub CS1 (Genv.globalenv p1)); trivial; apply JUST1.
                 trivial. trivial. apply Comp_ctx_LNR. }
               apply SF1. 
            ** specialize (FundefsMatch _ _ (eq_refl _) (eq_refl _)). simpl in FundefsMatch. 
               destruct FundefsMatch as [SIGs [psi2 PSI2]].
               rewrite Functions_preserved in H. inv H. simpl.
               assert (SF1: semaxfunc_InternalInfo cs (*(V1 ++ V2)*)V (Imports ++ G_merge (Comp_G c1) (Comp_G c2)) (Genv.globalenv p) i f phi1).
               { eapply InternalInfo_subsumption.
                 4: apply (InternalInfo_envs_sub CS1 (Genv.globalenv p1)); trivial; apply JUST1.
                 trivial. trivial. apply Comp_ctx_LNR. }
   ...
   - (*i in G2 but not in G1 -- symmetric*) 
      ...
      ++ destruct EF2 as [ef [tys [rt [cc EF2]]]]. apply Fundef_of_Gfun in EF2. specialize (JUST2 _ _ EF2 (eq_refl _)).
         ...
         -- clear FundefsMatch. rewrite EF2 in Functions_preserved. rewrite Functions_preserved in H. inv H. simpl.
            eapply ExternalInfo_envs_sub. 3: apply JUST2. trivial. trivial.
      ++ destruct IF2 as [f IF2]. apply Fundef_of_Gfun in IF2. rewrite IF2 in *. specialize (JUST2 _ _ (eq_refl _) (eq_refl _)).
         remember (find_id i (prog_funct p1)) as w1; symmetry in Heqw1; destruct w1.
         -- destruct f0.
            ** specialize (FundefsMatch _ _ (eq_refl _) (eq_refl _)). simpl in FundefsMatch. inv FundefsMatch. 
               rewrite Functions_preserved in H. inv H. 
               assert (SF2: semaxfunc_InternalInfo cs (*(V1 ++ V2)*)V (Imports ++ G_merge (Comp_G c1) (Comp_G c2)) (Genv.globalenv p) i f phi).
               { eapply InternalInfo_subsumption.
                 4: apply (InternalInfo_envs_sub CS2 (Genv.globalenv p2)); trivial; apply JUST2.
                 trivial. trivial. apply Comp_ctx_LNR. }
               apply SF2. 
...
Inductive semaxfunc {Espec} {cs : compspecs} (V : varspecs) (G : funspecs) (ge : Genv.t fundef type):
  list (ident * fundef) -> funspecs -> Prop :=
  semaxfunc_nil: @semaxfunc Espec cs V G ge nil nil

| semaxfunc_cons: forall (fs : list (ident * fundef)) (id : ident) (f : function) phi (G' : funspecs),
...
| semaxfunc_cons_ext: forall (fs : list (ident * fundef)) (id : ident) 
...
Proof.
  ...
  assert (LNRfuncs: list_norepet (map fst (prog_funct p)))
    by (apply list_norepet_prog_funct'; trivial).
  apply SF_semaxfunc.
...
Lemma Canonical_PROG {Espec cs V E} p Exp
      (c: @CanonicalComponent Espec V cs E nil p Exp)
